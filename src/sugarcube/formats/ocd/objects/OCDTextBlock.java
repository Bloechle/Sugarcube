...
public class OCDTextBlock extends OCDGroup<OCDTextLine> {
    ...
    public JsonMap toJson() {
        ...
        json.put("class", "Paragraph");
        json.put("type", "group");
        json.put("fontName", this.fontName());
        json.put("fontSize", this.meanFontsize());
        json.put("content", this.stringValue(true));

        writeBoundsToJson(json);

        json.put("size", this.nbOfLines());

        JsonArray linesArray = new JsonArray();
        for (OCDTextLine line : this) {
            linesArray.add(line.toJson());
        }
        json.put("textLines", linesArray);
