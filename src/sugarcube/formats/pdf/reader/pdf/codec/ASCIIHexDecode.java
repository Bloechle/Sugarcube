...
public class ASCIIHexDecode
{
    ...
    while (buf.remaining() > 0)
    {
      ...
      if (!PDF.isWhiteSpace((char) c))
      {
        if (c >= '0' && c <= '9')
          ...
          Log.warn(this, ".readHexDigit - unexpected character: " + c);
        return c;
      }
    }
    byte[] a = buf.array();
    Log.debug(this, ".readHexDigit - unexpected end of stream: " + asString(a, a.length - 8, a.length));
    return -1;
  }
  ...
  /**
   * decode an array of bytes in ASCIIHex format. <p> ASCIIHex format consists of a sequence of Hexidecimal digits, with possible whitespace, ending with the
   * '&gt;' character.
   *
   ...
   * @return the decoded bytes
   */
  public static byte[] decode(byte[] stream) throws Exception
  {
    return new ASCIIHexDecode(ByteBuffer.wrap(stream)).decode();
  }

  public static String asString(byte[] chars, int begin, int end)
  {
