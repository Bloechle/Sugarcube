 ...
 */
package sugarcube.formats.pdf.reader.pdf.node.font.ttf;
...
public class TTF
{
     ...
     */
    public void addTable(String tagString, TTFTable table)
    {
     ...
     * @param tagString the name of this table, as a 4 character string (i.e. cmap or head)
     */
    public TTFTable getTable(String tagString)
    {
        ...
        {
            // the table has not yet been parsed.  Parse it, and add the parsed version to the map of tables.
            ByteBuffer data = (ByteBuffer) tableObj;

            table = TTFTable.createTable(this, tagString, data);
            addTable(tagString, table);
        } else
            table = (TTFTable) tableObj;
    ...
    public byte[] writeFont()
    {
        ...
        for (Iterator i = tables.keySet().iterator(); i.hasNext(); )
        {
            ...
            int tag = TTFTable.stringToTag(tagString);

            ByteBuffer data = null;

            Object tableObj = tables.get(tagString);
            if (tableObj instanceof TTFTable)
                data = ((TTFTable) tableObj).getData();
            else
    ...
    private int getLength()
    {
        ...
        for (Iterator i = tables.values().iterator(); i.hasNext(); )
        {
            ...
            if (tableObj instanceof TTFTable)
                length += ((TTFTable) tableObj).getLength();
            else
    ...
    private void updateChecksumAdj(ByteBuffer fontData)
    {
        ...
        for (Iterator i = tables.keySet().iterator(); i.hasNext(); )
        {
            ...
            Object tableObj = tables.get(tagString);
            if (tableObj instanceof TTFTable)
                offset += ((TTFTable) tableObj).getLength();
            else
                                         ...
                                         TTF ttf)
    {
        ...
        {
            int tag = data.getInt();
            String tagString = TTFTable.tagToString(tag);
...
//                    checksum + ", offset: " + offset + ", length: " + length);
        ...
        /*
         * System.out.println("Mismatched checksums on table " + tagString + ": " + calcChecksum + " != " +
         checksum);
         */
                ttf.addTable(tagString, tableData);
            ...
            {
                Log.warn(TTF.class, ".parseDirectories - exception: " + e);
            }
        }
    }
    ...
    public static TTF ParseFont(byte[] bytes)
    {
        TTF font = null;
        try
        {
            font = ParseFont(ByteBuffer.wrap(bytes));
        } catch (Exception e)
        {
            Log.warn(TTF.class, ".ParseFont - error: " + e.getMessage());
            e.printStackTrace();
        }
        return font;
    }

    public static TTF ParseFont(ByteBuffer buffer)
    {
        int type = buffer.getInt();
        short numTables = buffer.getShort();
        short searchRange = buffer.getShort();
        short entrySelector = buffer.getShort();
        short rangeShift = buffer.getShort();

        TTF font = new TTF(type);
        ParseDirectories(buffer, numTables, font);
