...
public class CCITT1D
{
  ...
  public boolean isByteAligned = false;
  byte[] data;
  int bitReached;
  ...
  public CCITT1D(PDFImage pdfImage)
  {

    this.data = pdfImage.bytes();
    this.bitReached = 0;

    PDFDictionary map = pdfImage.decodeParms();

    width = pdfImage.width(); //default if not set (in theory should be the same if set)
    height = pdfImage.height();

    if (map != null)
    {
      int cols = map.get("Columns").intValue(width > 0 ? width : 1728);
      if (cols > -1)
        this.width = cols;
      int rows = map.get("Rows").intValue(height);
      if (rows > 0)
        this.height = rows;
      this.isByteAligned = map.get("EncodedByteAlign").booleanValue(false);
      //PDF default convention: 1 bit is white, 0 bit is black
      blackIs1 = map.get("BlackIs1").booleanValue(false);
    }

    scanlineStride = (width + 7) >> 3;
  ...
  private void decode1DRun()
  {
    ...
    while (!EOS)
    { //repeat until we hit the end of the data
  ...
  private int getCodeWord()
  {
    ...
    if (isWhite)
    { //no values for 2,3 in this case
      startBitLength = 4;
      endBit = 13;
    }
  ...
  private void moveToEOLMarker()
  {
    ...
    while (!isEOL)
    {
      ...
      for (i = 0; i < 12; i++)
        if (i == 11)
        {
          if (inputBits.get(i + bitReached) == false)
            isEOL = false;
        }
        else if (inputBits.get(i + bitReached) == true)
          isEOL = false;
