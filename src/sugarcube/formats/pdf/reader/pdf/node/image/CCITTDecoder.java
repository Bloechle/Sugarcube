...
{
  public static Image3 Decode(PDFImage pdfImage) throws Exception
  {
    PDFStream pdfStream = pdfImage.stream;
    byte[] encoded = pdfStream.byteValues();
    PDFDictionary parms = pdfImage.decodeParms();
    int k = parms.get("K").intValue(-1);

    int cols = parms.get("Columns").intValue(pdfImage.width > 0 ? pdfImage.width : 1728);
    int rows = parms.get("Rows").intValue(pdfImage.height);
    if(rows<=0)
      rows = pdfImage.height;

    byte[] decoded = new byte[rows * ((cols + 7) >> 3)];

    CCITTDecoder decoder = new CCITTDecoder(1, cols, rows);
    decoder.setAlign(parms.get("EncodedByteAlign").booleanValue(false));
    if (k == 0)
      decoder.decodeT41D(decoded, encoded, 0, rows);
    else if (k > 0)
      decoder.decodeT42D(decoded, encoded, 0, rows);
    else if (k < 0)
      decoder.decodeT6(decoded, encoded, 0, rows);    

    //PDF default convention: 1 bit is white, 0 bit is black
    boolean blackIs1 = parms.get("BlackIs1").booleanValue(false);

//    Log.debug(CCITTDecoder.class,  ".decode - "+pdfImage.stream.reference()+": k="+k+", blackIs1="+blackIs1+", decode[0]="+pdfImage.decode[0]);    
    
    for (int i = 0; i < decoded.length; i++)
      decoded[i] = (byte) ~decoded[i];

    //guess if resuting decoded image is byte aligned or not
    boolean isByteAligned = rows > 8 && decoded.length * 8 - 8 < rows * cols ? false : true;
    Image3 image = new Image3(cols, rows, pdfImage.imageMask ? Image3.Type.ARGB : Image3.Type.RGB);

    Color3 zero = Color3.BLACK;
    Color3 one = pdfImage.imageMask ? Color3.TRANSPARENT : Color3.WHITE;
    if (blackIs1 ^ pdfImage.decode[0] > 0)
    {
      Color3 tmp = zero;
      zero = one;
      one = tmp;
    ...
    for (int y = 0; y < image.height(); y++)
    {
      for (int x = 0; x < image.width(); x++)
      {
        if (index < decoded.length)
          image.setPixel(x, y, (decoded[index] & (1 << offset)) > 0 ? one : zero);
        if (offset == 0)
        {
          offset = 7;
          index++;
   ...
   * @param h
   */
  public CCITTDecoder(int fillOrder, int w, int h)
  {
    ...
    int startX, int height)
  {
    ...
    try
    {
      ...
      for (int lines = 0; lines < height; lines++)
      {
        ...
        if (currIndex <= w && currIndex < cce.length)
          cce[currIndex++] = bitOffset;
    ...
    {
      Log.debug(this, ".decodeT6 - hiccup: index=" + currIndex + ", changing=" + changingElemSize + ", size=" + cce.length);
    }
  }
