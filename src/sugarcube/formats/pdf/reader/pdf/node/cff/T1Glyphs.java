...
public class T1Glyphs extends PdfJavaGlyphs
{
  ...
  public String[] charForGlyphIndex;//Zoubi not used
  /**
   * holds mappings for drawing the glpyhs
   */
  public Map<String, byte[]> charStrings = new Hashtable<String, byte[]>();
  /**
   * holds the numbers
   */
  int max = 100;
   ...
   * "BlueValues", "OtherBlues", "FamilyBlues", "FamilyOtherBlues", "StdHW", "StdVW", "escape", "UniqueID", "XUID", "charset", "Encoding", "CharStrings",
   * "Private", "Subrs", "defaultWidthX", "nominalWidthX", "-Reserved-", "-Reserved-", "-Reserved-", "-Reserved-", "-Reserved-", "-Reserved-", "intint",
   * "longint", "BCD", "-Reserved-" };/*
   */
  /**
   * 2 byte ops in CFF DIct table * private static String[] raw2ByteValues = { "Copyright", "isFixedPitch", "ItalicAngle", "UnderlinePosition",
   * "UnderlineThickness", "PaintType", "CharstringType", "FontMatrix", "StrokeWidth", "BlueScale", "BlueShift", "BlueFuzz", "StemSnapH", "StemSnapV",
   * "ForceBold", "-Reserved-", "-Reserved-", "LanguageGroup", "ExpansionFactor", "initialRandomSeed", "SyntheticBase", "PostScript", "BaseFontName",
   * "BaseFontBlend", "-Reserved-", "-Reserved-", "-Reserved-", "-Reserved-", "-Reserved-", "-Reserved-", "ROS", "CIDFontVersion", "CIDFontRevision",
...
//  }
  ...
  private boolean processFlex(GlyphFactory factory, boolean debug, int lastKey, boolean isFlex, int routine)
  {
    ...
    { //determine if flex feature and enable
      isFlex = true;
      ptCount = 0;
      pt = new float[16];
      if (debug)
  ...
  private void hsbw(GlyphFactory factory, String glyphName, boolean debug)
  {
     ...
     * dynamicVectorRenderer.getType()==HTMLDisplay.CREATE_HTML) { //Just to be safe HTMLDisplay display = ((HTMLDisplay)dynamicVectorRenderer);
     *
     * display.saveAdvanceWidth(baseFontName,glyphName,(int)ys); } //<end-std> /*
     */
    allowAll = true;
  }
  ...
  public Path3 getPath(String name, String uni, int code)
  {
    ...
    x = -factory.getLSB();
    y = 0;
    decodeGlyph(null, factory, name, code, uni, -1, false);
    return factory.hasGlyph ? factory.getPath() : null;
  }

  /*
   * creates glyph from type1c font commands
   */
  protected void decodeGlyph(String embKey, GlyphFactory factory, String glyphName, int rawInt, String displayValue, float currentWidth, boolean isRecursive)
  {
     ...
     */
//    Log.debug(this, ".decodeGlyph - glyph=" + glyphName + ", rawInt=" + rawInt + ", displayValue=" + displayValue+", chars"+charStrings.keySet());
    if (isCID)
      glyphStream = (byte[]) charStrings.get(String.valueOf(rawInt));
    else
    {
     
      if (glyphName==null || glyphName.equals(".notdef") || !charStrings.containsKey(glyphName))
      {
        if (displayValue!=null && charStrings.containsKey(displayValue))
          glyphName = displayValue;
        else if (charStrings.containsKey("" + (char) rawInt))
          glyphName = "" + (char) rawInt;
        else if (charStrings.containsKey("C" + (int) rawInt))
          glyphName = "C" + (int) rawInt;
       ...
       */
      glyphStream = (byte[]) charStrings.get(glyphName);
    ...
    else
    {
      ...
      if ((debug))
      {
        System.out.println("******************" + ' ' + displayValue + ' ' + glyphName);
      ...
      while (p < dicEnd)
      {
        ...
        {  //if its a number get it and update pointer p
        ...
        {  // operator
           ...
           * //<end-pro>
           *
           * //<start-std> //This tests whether the previously saved potential width is an argument of the first operator or //an actual width. If it's an
           * actual width, it's saved. if (commandCount == -1 && //Only on first command operandReached % 2 == 1 && //Make sure not an argument baseFontName !=
           * null && //Check right call dynamicVectorRenderer != null && //Check right call dynamicVectorRenderer.getType()== HTMLDisplay.CREATE_HTML) { //Just
           * to be safe HTMLDisplay display = ((HTMLDisplay)dynamicVectorRenderer);
           *
           * display.saveAdvanceWidth(baseFontName,glyph,potentialWidth); } //<end-std> /*
           */
          commandCount++;
          isNumber = false;
          lastKey = key;
          ...
          if (key == 12)
          { //handle escaped keys (ie 2 byte ops)
            ...
            { //sbw
              ...
              {
                operandReached = 0; //move to first operator
                System.out.println("1 Not implemented " + p + " id=" + key + " op=" + key);
              }
              else
                operandReached = 0; //move to first operator
          }
          ...
          { //hsbw (T1 only)
            ...
            { //callsubr and callgsubr
              ...
              if (debug)
                System.out.println(key + " -------------- last Value=" + lastVal + ' ' + allowAll + " commandCount=" + commandCount + " operandReached=" + operandReached + " " + isFirst);
              ...
              if (1 == 2 && is1C && isFirst)
              {
                ...
                if (debug)
                  System.out.println("reset xs,ys");
              }
...
//								factor in bias
                ...
                if (newStream != null)
                {
                   ...
                   *
                   * System.out.println("Merged-------------"); for(int p2=0;p2<glyphStream.length;p2++){ if(p2==lastNumberStart ||
                   * p==(lastNumberStart+newLength)) System.out.println("----"); System.out.println(p2+" "+(glyphStream[p2] & 0xff)); } } /*
                   */
                }
                else if (debug)
            ...
            { //unknown command
              ...
              System.out.println("Unsupported command " + p + ">>>>>" + hintCount + ">>>>>>key=" + key + ' ' + key + " <1<<" + operandsRead);
              for (int j = 0; j < dicEnd; j++)
                System.out.println(j + " " + (glyphStream[j] & 0xff));
      ...
      {//added for M2003W.pdf font display
        //if(ymin<0)
        //	ymin=270;
      }
      else
