...
{
  public static final String RES = "sugarcube/formats/pdf/resources/pdf/cfg/";
  /**
  ...
  /**
   * names of CID fonts supplied by Adobe* private static String[] CIDFonts = { "83pv-RKSJ-H", "90msp-RKSJ-H", "90msp-RKSJ-V", "90ms-RKSJ-H", "90ms-RKSJ-UCS2",
   * "90ms-RKSJ-V", "90pv-RKSJ-H", "90pv-RKSJ-UCS2", "90pv-RKSJ-UCS2C", "Add-RKSJ-H", "Add-RKSJ-V", "Adobe-CNS1-3", "Adobe-CNS1-UCS2", "Adobe-GB1-4",
   * "Adobe-GB1-UCS2", "Adobe-Japan1-4", "Adobe-Japan1-UCS2", "Adobe-Korea1-2", "Adobe-Korea1-UCS2", "B5pc-H", "B5pc-UCS2", "B5pc-UCS2C", "B5pc-V", "CNS-EUC-H",
   * "CNS-EUC-V", "ETen-B5-H", "ETen-B5-UCS2", "ETen-B5-V", "euc-h", "euc-v", "Ext-RKSJ-H", "Ext-RKSJ-V", "gb-euc-h", "gb-euc-v", "gbk2k-h", "gbk2k-v",
   * "GBK-EUC-H", "GBK-EUC-UCS2", "GBK-EUC-V", "GBKp-EUC-H", "GBKp-EUC-V", "GBpc-EUC-H", "GBpc-EUC-UCS2", "GBpc-EUC-UCS2C", "GBpc-EUC-V", "GBT-EUC-H",
   * "GBT-EUC-V", "h", "HKscs-B5-H", "HKscs-B5-V", "KSC-EUC-H", "KSC-EUC-V", "KSCms-UHC-H", "KSCms-UHC-HW-H", "KSCms-UHC-HW-V", "KSCms-UHC-UCS2", "KSCms-UHC-V",
   * "KSCpc-EUC-H", "KSCpc-EUC-UCS2", "KSCpc-EUC-UCS2C", "UniCNS-UCS2-H", "UniCNS-UCS2-V", "UniGB-UCS2-H", "UniGB-UCS2-V", "UniJIS-UCS2-H", "UniJIS-UCS2-HW-H",
   * "UniJIS-UCS2-HW-V", "UniJIS-UCS2-V", "UniKS-UCS2-H", "UniKS-UCS2-V", "v" };/*
   */
  /**
    ...
    String file_name)
  {
    ...
    try
    {
      input_stream = (file_name.equals("symbol.cfg"))
        ? new BufferedReader(new InputStreamReader(loader.getResourceAsStream(RES + file_name), enc))
        : new BufferedReader(new InputStreamReader(loader.getResourceAsStream(RES + file_name), "UTF-16"));

      // trap problems
      if (input_stream == null)
        Log.warn(StandardFonts.class, "Unable to open " + file_name + " to read standard encoding");
  ...
  final static private void readStandardMappingTable(int idx)
  {
    ...
    try
    {
      ...
      input_stream = new BufferedReader(new InputStreamReader(loader.getResourceAsStream(RES + "standard_encoding.cfg"), enc));
      usesGlyphlist = true;

      // trap problems
      if (input_stream == null)
        Log.warn(StandardFonts.class, "Unable to open standard_encoding.cfg from jar");
   ...
   * String s=null;
   *
   * //Create the encoder and decoder for ISO-8859-1 Charset charset = Charset.forName(enc); CharsetDecoder decoder = charset.newDecoder(); CharsetEncoder
   ...
   * encoder.encode(CharBuffer.wrap("a")); bbuf.clear(); //java.nio.ByteBuffer bbuf=new ByteBuffer(); bbuf.put(0,(byte) Integer.parseInt(value,16)); // Convert
   * ISO-LATIN-1 bytes in a ByteBuffer to a character ByteBuffer and then to a string. // The new ByteBuffer is ready to be read. CharBuffer cbuf =
   * decoder.decode(bbuf); s = cbuf.toString();
   *
  ...
  final private static void loadStandardFont(int i) throws IOException
  {
    ...
    {
      //open the file
      input_stream = new BufferedReader(new InputStreamReader(loader.getResourceAsStream(RES + "defaults/" + files_names[i] + ".afm"), enc));

      boolean char_mapping_table = false;
      //flag if in correct part of file
      while (true)
      { //read the lines and extract width info
        line = input_stream.readLine();
    ...
    if (adobeMap == null)
      try
      {
        ...
        input_stream = new BufferedReader(
          new InputStreamReader(loader.getResourceAsStream(RES + "glyphlist.cfg"), enc));

        // trap problems
        if (input_stream == null)
          Log.warn(StandardFonts.class, "Unable to open glyphlist.txt from jar");
...
//  }
  ...
  public static String convertNumberToGlyph(String mappedChar, boolean containsHexNumbers, boolean allowSingleValue)
  {
    ...
    if (charCount == 2 || charCount == 3)
    {
      ...
      { //test all values to see if number
        char c = mappedChar.charAt(ii);
        if (c < '0' || c > '9')
        {  //fail on first and exit loop
          isNumber = false;
          ii = charCount;
        }
      }
