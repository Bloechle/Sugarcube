...
/**
 * This class represents the access permissions to a document. These permissions
 * are specified in the PDF format specifications, they include:
 * <ul>
 ...
 * <li>extract text and graphics for accessibility to visually impaired
 * people</li>
 ...
 * to recipients. In this case, it must be used with a specific
 * ProtectionPolicy.
 *
 *
...
public class AccessPermission
{
  ...
  /**
   * Create a new access permission object. By default, all permissions are
   * granted.
   */
  ...
  /**
   * Create a new access permission object from a byte array. Bytes are ordered
   ...
   *
   * @return true if the access permission does not restrict the use of the
   *         document
   */
  public boolean isOwnerPermission()
  {
    return (this.canAssembleDocument() && this.canExtractContent() && this.canExtractForAccessibility() && this.canFillInForm() && this.canModify()
  ...
  /**
   * This returns an integer representing the access permissions. This integer
   * can be used for public key encryption. This format is not documented in the
   * PDF specifications but is necessary for compatibility with Adobe Acrobat
   * and Adobe Reader.
   *
  ...
  /**
   * The returns an integer representing the access permissions. This integer
   * can be used for standard PDF encryption as specified in the PDF
   * specifications.
   *
   ...
   *
   * @return true If supplied with the user password they are allowed to modify
   *         the document
   */
  ...
  /**
   * This will tell if the user can extract text and images from the PDF
   * document.
   *
   * @return true If supplied with the user password they are allowed to extract
   ...
   *          A boolean determining if the user can extract content from the
   *          document.
   */
   ...
   * interactive forms fields and, if {@link #canModify() canModify()} returns
   * true, create or modify interactive form fields (including signature
   * fields). Note that if {@link #canFillInForm() canFillInForm()} returns
   * true, it is still possible to fill in interactive forms (including
   ...
   * or modify interactive form fields (including signature fields). Note that
   * if {@link #canFillInForm() canFillInForm()} returns true, it is still
   * possible to fill in interactive forms (including signature fields) even the
   * parameter here is false.
   * <p>
   * This method will have no effect if the object is in read only mode.
   *
   * @param allowAnnotationModification
   *          A boolean determining the new setting.
   */
   ...
   * signature fields) even if {@link #canModifyAnnotations()
   * canModifyAnnotations()} returns false.
   *
   * @return true If supplied with the user password they are allowed to fill in
   *         form fields.
   */
   ...
   * fields) even if {@link #canModifyAnnotations() canModifyAnnotations()}
   * returns false. Therefore, if you want to prevent a user from filling in
   * interactive form fields, you need to call
   * {@link #setCanModifyAnnotations(boolean) setCanModifyAnnotations(false)} as
   * well.
   * <p>
   * This method will have no effect if the object is in read only mode.
   *
   * @param allowFillingInForm
  ...
  /**
   * This will tell if the user can extract text and images from the PDF
   * document for accessibility purposes.
   *
   * @return true If supplied with the user password they are allowed to extract
   *         content from the PDF document
   */
   ...
   * This method will have no effect if the object is in read only mode.
   *
   * @param canPrintDegraded
   *          A boolean determining if the user can print the document in a
   *          degraded format.
   */
  ...
  /**
   * Locks the access permission read only (ie, the setters will have no
   * effects). After that, the object cannot be unlocked. This method is used
   * for the currentAccessPermssion of a document to avoid users to change
   * access permission.
   */
