...
import sugarcube.formats.pdf.reader.pdf.codec.Charsets;
import sugarcube.formats.pdf.reader.pdf.encryption.EncryptionDico;
...
/**
 * The standard security handler. This security handler protects document with
 * password.
 * 
 * @see StandardProtectionPolicy to see how to protect document with this
 *      security handler.
...
public class StandardSecurityHandler extends SecurityHandler
{
  ...
  { (byte) 0x28, (byte) 0xBF, (byte) 0x4E, (byte) 0x5E, (byte) 0x4E, (byte) 0x75, (byte) 0x8A, (byte) 0x41, (byte) 0x64, (byte) 0x00, (byte) 0x4E,
      (byte) 0x56, (byte) 0xFF, (byte) 0xFA, (byte) 0x01, (byte) 0x08, (byte) 0x2E, (byte) 0x2E, (byte) 0x00, (byte) 0xB6, (byte) 0xD0, (byte) 0x68,
      (byte) 0x3E, (byte) 0x80, (byte) 0x2F, (byte) 0x0C, (byte) 0xA9, (byte) 0xFE, (byte) 0x64, (byte) 0x53, (byte) 0x69, (byte) 0x7A };

  // hashes used for Algorithm 2.B, depending on remainder from E modulo 3
  private static final String[] HASHES_2B = new String[]
  { "SHA-256", "SHA-384", "SHA-512" };

  private static final int DEFAULT_VERSION = 1;

  private StandardProtectionPolicy policy;
  private EncryptionDico encryption;

  public StandardSecurityHandler(EncryptionDico encryption)
  {   
    try {
...
//    StandardDecryptionMaterial material = (StandardDecryptionMaterial) decryptionMaterial;
    ...
    byte[] documentIDBytes = encryption.idBytes;

    // we need to know whether the meta data was encrypted for password
    // calculation
    boolean encryptMetadata = encryption.encryptMetadata;
    ...
    if (isOwnerPassword(password.getBytes(passwordCharset), userKey, ownerKey, dicPermissions, documentIDBytes, dicRevision, dicLength,
        encryptMetadata))
    {
      ...
      encryptionKey = computeEncryptedKey(computedPassword, ownerKey, userKey, oe, ue, dicPermissions, documentIDBytes, dicRevision, dicLength,
        ...
        encryptMetadata))
    {
      encryptionKey = computeEncryptedKey(password.getBytes(passwordCharset), ownerKey, userKey, oe, ue, dicPermissions, documentIDBytes, dicRevision,
          dicLength, encryptMetadata, false);
    } else
    {
      throw new RuntimeException("Cannot decrypt PDF, the password is incorrect");
    }
    ...
    {
      // detect whether AES encryption is used. This assumes that the encryption
      // algo is
      // stored in the PDCryptFilterDictionary
      // However, crypt filters are used only when V is 4 or 5.


      setAES(encryption.isAESV2or3());
  ...
  /**
   * Computes the version number of the StandardSecurityHandler regarding the
   * encryption key length. See PDF Spec 1.6 p 93 and PDF 1.7 AEL3
   *
  ...
  /**
   * Computes the revision version of the StandardSecurityHandler to use
   * regarding the version number and the permissions bits set. See PDF Spec 1.6
   * p98
   * 
   * @param version
   *          The version number.
  ...
  // Algorithm 13: validate permissions ("Perms" field). Relaxed to accomodate
  // buggy encoders
  // https://www.adobe.com/content/dam/Adobe/en/devnet/acrobat/pdfs/adobe_supplement_iso32000.pdf
  private void validatePerms(EncryptionDico encryption, int dicPermissions, boolean encryptMetadata) throws IOException
  {
    try
    {
      ...
      byte[] perms = cipher.doFinal(encryption.perms);

      // "Verify that bytes 9-11 of the result are the characters ‘a’, ‘d’,
      // ‘b’."
      if (perms[9] != 'a' || perms[10] != 'd' || perms[11] != 'b')
      {
        Log3.warn(this, "Verification of permissions failed (constant)");
      }
      ...
      {
        Log3.warn(this,
            "Verification of permissions failed (" + String.format("%08X", permsP) + " != " + String.format("%08X", dicPermissions) + ")");
      }

      if (encryptMetadata && perms[8] != 'T' || !encryptMetadata && perms[8] != 'F')
      {
        Log3.warn(this, "Verification of permissions failed (EncryptMetadata)");
      }
   ...
   */
  public boolean isOwnerPassword(byte[] ownerPassword, byte[] user, byte[] owner, int permissions, byte[] id, int encRevision, int length,
   ...
   *
   * @throws IOException
   *           If there is an error accessing data while generating the user
   *           password.
   */
   ...
   */
  public byte[] computeEncryptedKey(byte[] password, byte[] o, byte[] u, byte[] oe, byte[] ue, int permissions, byte[] id, int encRevision,
      int length, boolean encryptMetadata, boolean isOwnerPassword) throws IOException
  {
    if (encRevision == 6 || encRevision == 5)
    {
  ...
  private byte[] computeEncryptedKeyRev234(byte[] password, byte[] o, int permissions, byte[] id, boolean encryptMetadata, int length,
      int encRevision)
  {
  ...
  private byte[] computeEncryptedKeyRev56(byte[] password, boolean isOwnerPassword, byte[] o, byte[] u, byte[] oe, byte[] ue, int encRevision)
      throws IOException
  {
   ...
   */
  public byte[] computeUserPassword(byte[] password, byte[] owner, int permissions, byte[] id, int encRevision, int length, boolean encryptMetadata)
      throws IOException
  {
    ByteArrayOutputStream result = new ByteArrayOutputStream();
  ...
  // steps (a) to (d) of "Algorithm 3: Computing the encryption dictionary?s O
   ...
   *           If there is an error accessing data.
   */
  public boolean isUserPassword(byte[] password, byte[] user, byte[] owner, int permissions, byte[] id, int encRevision, int length,
      boolean encryptMetadata) throws IOException
  {
    switch (encRevision)
    {
    ...
    case 3:
    case 4:
      return isUserPassword234(password, user, owner, permissions, id, encRevision, length, encryptMetadata);
    case 5:
    case 6:
  ...
  private boolean isUserPassword234(byte[] password, byte[] user, byte[] owner, int permissions, byte[] id, int encRevision, int length,
      boolean encryptMetadata) throws IOException
  {
   ...
   *           If there is an error accessing data.
   */
  public boolean isUserPassword(String password, byte[] user, byte[] owner, int permissions, byte[] id, int encRevision, int length,
      boolean encryptMetadata) throws IOException
  {
    if (encRevision == 6 || encRevision == 5)
    {
      return isUserPassword(password.getBytes(Charsets.UTF_8), user, owner, permissions, id, encRevision, length, encryptMetadata);
    } else
    {
      return isUserPassword(password.getBytes(Charsets.ISO_8859_1), user, owner, permissions, id, encRevision, length, encryptMetadata);
    }
  }
   ...
   *           If there is an error accessing data.
   */
  public boolean isOwnerPassword(String password, byte[] user, byte[] owner, int permissions, byte[] id, int encRevision, int length,
      boolean encryptMetadata) throws IOException
  {
    return isOwnerPassword(password.getBytes(Charsets.ISO_8859_1), user, owner, permissions, id, encRevision, length, encryptMetadata);
  }
  ...
  private static void logIfStrongEncryptionMissing()
  {
    try
    {
      ...
      {
        Log3.warn(StandardSecurityHandler.class, "JCE unlimited strength jurisdiction policy files are not installed");
      }
