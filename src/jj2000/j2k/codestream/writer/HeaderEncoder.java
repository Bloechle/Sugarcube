...
/**
 * This class writes almost of the markers and marker segments in main header and in tile-part headers. It is created by the run() method of the Encoder
 * instance.
 *
 * <p>A marker segment includes a marker and eventually marker segment parameters. It is designed by the three letter code of the marker associated with the
 * marker segment. JPEG 2000 part I defines 6 types of markers: <ul> <li>Delimiting : SOC,SOT,SOD,EOC (written in FileCodestreamWriter).</li> <li>Fixed
 * information: SIZ.</li> <li>Functional: COD,COC,RGN,QCD,QCC,POC.</li> <li> In bit-stream: SOP,EPH.</li> <li> Pointer: TLM,PLM,PLT,PPM,PPT.</li> <li>
 * Informational: CRG,COM.</li> </ul></p>
 *
 * <p>Main Header is written when Encoder instance calls encodeMainHeader whereas tile-part headers are written when the EBCOTRateAllocator instance calls
 * encodeTilePartHeader.</p>
 *
 * @see Encoder
 * @see Markers
...
public class HeaderEncoder implements Markers, StdEntropyCoderOptions
{
  ...
  /**
   * The list of parameters that are accepted for the header encoder module. Options for this modules start with 'H'.
   */
  ...
  /**
   * The ByteArrayOutputStream to store header data. This handler is kept in order to use methods not accessible from a general DataOutputStream. For the other
   * methods, it's better to use variable hbuf.
   *
   * @see #hbuf
   */
   ...
   *
   * @param origsrc The original image data (before any component mixing, tiling, etc.)
   *
   * @param isorigsig An array specifying for each component if it was originally signed or not.
   *
    ...
    ParameterList pl)
  {
    ...
    enJJ2KMarkSeg = pl.bool("Hjj2000_COM");
    otherCOMMarkSeg = pl.value("HCOM");
  }

  /**
   * Resets the contents of this HeaderEncoder to its initial state. It erases all the data in the header buffer and reactualizes the headerLength field of the
   * bit stream writer.
     *
   */
  public void reset()
  ...
  /**
   * Writes SIZ marker segment of the codestream header. It is a fixed information marker segment containing informations about image and tile sizes. It is
  ...
  /**
   * Writes COD marker segment. COD is a functional marker segment containing the code style default (coding style, decomposition, layering) used for
   * compressing all the components in an image.
   *
   * <p>The values can be overriden for an individual component by a COC marker in either the main or the tile header.</p>
   *
   * @param mh Flag indicating whether this marker belongs to the main header
   *
  ...
  protected void writeCOD(boolean mh, int tileIdx) throws IOException
  {
    ...
    if (mh)
    {
      ...
      if (((String) encSpec.ephs.getTileDef(tileIdx).toString())
        ...
        tmp |= SCOX_USE_EPH;
    if (dwt.getCbULX() != 0)
      tmp |= SCOX_HOR_CB_PART;
    if (dwt.getCbULY() != 0)
      tmp |= SCOX_VER_CB_PART;
    hbuf.write(tmp);
   ...
   *
   * <p>Its values overrides any value previously set in COD in the main header or in the tile header.</p>
   *
   * @param mh Flag indicating whether the main header is to be written.
   *
   * @param tileIdx Tile index.
   *
   * @param compIdx index of the component which need use of the COC marker segment.
   *
   * @see #writeCOD
     *
   */
  ...
  /**
   * Writes QCD marker segment in main header. QCD is a functional marker segment countaining the quantization default used for compressing all the components
   * in an image. The values can be overriden for an individual component by a QCC marker in either the main or the tile header.
     *
   */
  ...
  /**
   * Writes QCC marker segment in main header. It is a functional marker segment countaining the quantization used for compressing the specified component in an
   * image. The values override for the specified component what was defined by a QCC marker in either the main or the tile header.
   *
  ...
  /**
   * Writes QCD marker segment in tile header. QCD is a functional marker segment countaining the quantization default used for compressing all the components
   * in an image. The values can be overriden for an individual component by a QCC marker in either the main or the tile header.
   *
  ...
  /**
   * Writes QCC marker segment in tile header. It is a functional marker segment countaining the quantization used for compressing the specified component in an
   * image. The values override for the specified component what was defined by a QCC marker in either the main or the tile header.
   *
   * @param t Tile index
   *
  ...
  /**
   * Writes POC marker segment. POC is a functional marker segment containing the bounds and progression order for any progression order other than default in
   * the codestream.
   *
   * @param mh Flag indicating whether the main header is to be written
   *
   ...
   *
   * <p>This marker is currently written in main header and indicates the JJ2000 encoder's version that has created the codestream.</p>
     *
   */
   ...
   *
   * <p>May be used in tile or main header. If used in main header, it refers to a ROI of the whole image, regardless of tiling. When used in tile header, only
   * the particular tile is affected.</p>
   *
   ...
   *
   * <ol> <li>SOT</li> <li>COD (if needed)</li> <li>COC (if needed)</li> <li>QCD (if needed)</li> <li>QCC (if needed)</li> <li>RGN (if needed)</li> <li>POC (if
   * needed)</li> <li>SOD</li> </ol>
   *
