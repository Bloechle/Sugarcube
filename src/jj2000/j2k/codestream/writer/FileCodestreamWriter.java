...
/**
 * This class implements a CodestreamWriter for Java streams. The streams can be files or network connections, or any other resource that presents itself as a
 * OutputStream. See the CodestreamWriter abstract class for more details on the implementation of the CodestreamWriter abstract class.
 *
 * <p>Before any packet data is written to the bit stream (even in simulation mode) the complete header should be written otherwise incorrect estimates are
 * given by getMaxAvailableBytes() for rate allocation.
 *
  ...
  implements Markers
{
  ...
  /**
   * Opens the file 'file' for writing the codestream. The magic number is written to the bit stream. Normally, the header encoder must be empty (i.e. no data
   * has been written to it yet). A BufferedOutputStream is used on top of the file to increase throughput, the length of the buffer is DEF_BUF_LEN.
   *
   * @param file The file where to write the bit stream
   *
   * @param mb The maximum number of bytes that can be written to the bit stream.
   *
   * @exception IOException If an error occurs while trying to open the file for writing or while writing the magic number.
     *
   */
  ...
  /**
   * Opens the file named 'fname' for writing the bit stream, using the 'he' header encoder. The magic number is written to the bit stream. Normally, the header
   * encoder must be empty (i.e. no data has been written to it yet). A BufferedOutputStream is used on top of the file to increase throughput, the length of
   * the buffer is DEF_BUF_LEN.
   *
  ...
  /**
   * Uses the output stream 'os' for writing the bit stream, using the 'he' header encoder. The magic number is written to the bit stream. Normally, the header
   * encoder must be empty (i.e. no data has been written to it yet). No BufferedOutputStream is used on top of the output stream 'os'.
   *
   * @param os The output stream where to write the bit stream.
   *
   * @param mb The maximum number of bytes that can be written to the bit stream.
   *
   * @exception IOException If an error occurs while writing the magic number to the 'os' output stream.
     *
   */
  ...
  /**
   * Returns the number of bytes remaining available in the bit stream. This is the maximum allowed number of bytes minus the number of bytes that have already
   * been written to the bit stream. If more bytes have been written to the bit stream than the maximum number of allowed bytes, then a negative value is
   * returned.
   *
  ...
  /**
   * Writes a packet head to the bit stream and returns the number of bytes used by this header. It returns the total number of bytes that the packet head takes
   * in the bit stream. If in simulation mode then no data is written to the bit stream but the number of bytes is calculated. This can be used for iterative
   * rate allocation.
   *
   * <P>If the length of the data that is to be written to the bit stream is more than the space left (as returned by getMaxAvailableBytes()) only the data that
   * does not exceed the allowed length is written, the rest is discarded. However the value returned by the method is the total length of the packet, as if all
   * of it was written to the bit stream.
   *
   * <P>If the bit stream header has not been commited yet and 'sim' is false, then the bit stream header is automatically commited (see commitBitstreamHeader()
   ...
   *
   * @param sim Simulation mode flag. If true nothing is written to the bit stream, but the number of bytes that would be written is returned.
   *
   * @param sop Start of packet header marker flag. This flag indicates whether or not SOP markers should be written. If true, SOP markers should be written, if
   * false, they should not.
   *
   * @param eph End of Packet Header marker flag. This flag indicates whether or not EPH markers should be written. If true, EPH markers should be written, if
   * false, they should not.
   *
  ...
  /**
   * Writes a packet body to the bit stream and returns the number of bytes used by this body .If in simulation mode then no data is written to the bit stream
   * but the number of bytes is calculated. This can be used for iterative rate allocation.
   *
   * <P>If the length of the data that is to be written to the bit stream is more than the space left (as returned by getMaxAvailableBytes()) only the data that
   * does not exceed the allowed length is written, the rest is discarded. However the value returned by the method is the total length of the packet body , as
   * if all of it was written to the bit stream.
   *
   ...
   *
   * @param roiLen Number of byte to read in packet body to get all the ROI information
   *
