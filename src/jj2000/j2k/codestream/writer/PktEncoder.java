...
/**
 * This class builds packets and keeps the state information of packet interdependencies. It also supports saving the state and reverting (restoring) to the
 * last saved state, with the save() and restore() methods.
 *
 * <p>Each time the encodePacket() method is called a new packet is encoded, the packet header is returned by the method, and the packet body can be obtained
 * with the getLastBodyBuf() and getLastBodyLen() methods.</p>
 *
 */
public class PktEncoder
{
  ...
  /**
   * The tag tree for inclusion information. The indexes are outlined below. Note that the layer indexes start at 1, therefore, the layer index minus 1 is used.
   * The subband indices are used as they are defined in the Subband class. The tile indices start at 0 and follow a lexicographical order.
   *
   * <ul> <li>1st index: tile index, in lexicographical order</li> <li>2nd index: component index </li> <li>3rd index: resolution level </li> <li>4th index:
   * precinct index </li> <li>5th index: subband index </li> </ul>
     *
   */
  private TagTreeEncoder ttIncl[][][][][];
  /**
   * The tag tree for the maximum significant bit-plane. The indexes are outlined below. Note that the layer indexes start at 1, therefore, the layer index
   * minus 1 is used. The subband indices are used as they are defined in the Subband class. The tile indices start at 0 and follow a lexicographical order.
   *
   * <ul> <li>1st index: tile index, in lexicographical order</li> <li>2nd index: component index </li> <li>3rd index: resolution level </li> <li>4th index:
   * precinct index </li> <li>5th index: subband index</li> </ul>
     *
  ...
  /**
   * The base number of bits for sending code-block length information (referred as Lblock in the JPEG 2000 standard). The indexes are outlined below. Note that
   * the layer indexes start at 1, therefore, the layer index minus 1 is used. The subband indices are used as they are defined in the Subband class. The tile
   * indices start at 0 and follow a lexicographical order.
   *
   * <ul> <li>1st index: tile index, in lexicographical order </li> <li>2nd index: component index </li> <li>3rd index: resolution level </li> <li>4th index:
   * subband index </li> <li>5th index: code-block index, in lexicographical order</li> </ul>
     *
   */
  private int lblock[][][][][];
  /**
   * The last encoded truncation point for each code-block. A negative value means that no information has been included for the block, yet. The indexes are
   * outlined below. The subband indices are used as they are defined in the Subband class. The tile indices start at 0 and follow a lexicographical order. The
   * code-block indices follow a lexicographical order within the subband tile.
   *
   * <P>What is actually stored is the index of the element in CBlkRateDistStats.truncIdxs that gives the real truncation point.
   *
   * <ul> <li>1st index: tile index, in lexicographical order </li> <li>2nd index: component index </li> <li>3rd index: resolution level </li> <li>4th index:
   * subband index</li> <li>5th index: code-block index, in lexicographical order </li> </ul>
     *
  ...
  /**
   * The saved base number of bits for sending code-block length information. It is used for restoring previous saved state by restore(). The indexes are
   * outlined below. Note that the layer indexes start at 1, therefore, the layer index minus 1 is used. The subband indices are used as they are defined in the
   * Subband class. The tile indices start at 0 and follow a lexicographical order.
   *
   * <ul> <li>1st index: tile index, in lexicographical order </li> <li>2nd index: component index </li> <li>3rd index: resolution level </li> <li>4th index:
   * subband index </li> <li>5th index: code-block index, in lexicographical order</li> </ul>
     *
  ...
  /**
   * The saved last encoded truncation point for each code-block. It is used for restoring previous saved state by restore(). A negative value means that no
   * information has been included for the block, yet. The indexes are outlined below. The subband indices are used as they are defined in the Subband class.
   * The tile indices start at 0 and follow a lexicographical order. The code-block indices follow a lexicographical order within the subband tile.
   *
   * <ul> <li>1st index: tile index, in lexicographical order </li> <li>2nd index: component index </li> <li>3rd index: resolution level </li> <li>4th index:
   * subband index </li> <li>5th index: code-block index, in lexicographical order </li> </ul>
     *
   ...
   *
   * <ul> <li> 1st dim: tile index.</li> <li> 2nd dim: component index.</li> <li> 3rd dim: resolution level index.</li> <li> 4th dim: precinct index.</li> </ul> 
     *
   */
   ...
   *
   * @param numPrec Maximum number of precincts in each tile, component and resolution level.
   *
  ...
  private void fillPrecInfo(int t, int c, int r)
  {
    ...
    for (int i = istart; i <= iend; i++) // Vertical precincts
        ...
        { // LL subband
          ...
          else
          {
            ...
            for (int k = kstart; k <= kend; k++) // Vertical cblks
              ...
              } // Horizontal code-blocks // Vertical code-blocks
          }
        }
        else
      ...
      } // Horizontal precincts // Vertical precincts
  }

  /**
   * Encodes a packet and returns the buffer containing the encoded packet header. The code-blocks appear in a 3D array of CBlkRateDistStats, 'cbs'. The first
   * index is the tile index in lexicographical order, the second index is the subband index (as defined in the Subband class), and the third index is the
   * code-block index (whithin the subband tile) in lexicographical order as well. The indexes of the new truncation points for each code-block are specified by
   * the 3D array of int 'tIndx'. The indices of this array are the same as for cbs. The truncation point indices in 'tIndx' are the indices of the elements of
   * the 'truncIdxs' array, of the CBlkRateDistStats class, that give the real truncation points. If a truncation point index is negative it means that the
   * code-block has not been included in any layer yet. If the truncation point is less than or equal to the highest truncation point used in previous layers
   * then the code-block is not included in the packet. Otherwise, if larger, the code-block is included in the packet. The body of the packet can be obtained
   * with the getLastBodyBuf() and getLastBodyLen() methods.
   *
   * <p>Layers must be coded in increasing order, in consecutive manner, for each tile, component and resolution level (e.g., layer 1, then layer 2, etc.). For
   * different tile, component and/or resolution level no particular order must be followed.</p>
   *
   ...
   *
   * @param hbuf The header buffer. If null a new BitOutputBuffer is created and returned. This buffer is reset before anything is written to it.
   *
   * @param bbuf The body buffer. If null a new one is created. If not large enough a new one is created.
   *
    ...
    byte bbuf[], int pIdx)
  {
    ...
    { // Loop on subbands
      ...
      for (int m = 0; m < prec.cblk[s].length; m++) // Vertical code-blocks
            ...
            switch (newtp)
            {
              ...
              default:
                if (newtp <= 35)
                  ...
                  throw new ArithmeticException("Maximum number "
                    + "of truncation "
                    + "points exceeded");
            }
          }
        ...
        } // End loop on horizontal code-blocks // End loop on vertical code-blocks
    } // End loop on subband
  ...
  /**
   * Returns the buffer of the body of the last encoded packet. The length of the body can be retrieved with the getLastBodyLen() method. The length of the
   * array returned by this method may be larger than the actual body length.
   *
   * @return The buffer of body of the last encoded packet.
   *
   * @exception IllegalArgumentException If no packet has been coded since last reset(), last restore(), or object creation.
   *
   * @see #getLastBodyLen
     *
   */
        ...
        for (int r = lblock_t_c.length - 1; r >= 0; r--)
        {
          ...
          for (int p = ppinfo[t][c][r].length - 1; p >= 0; p--)
            if (p < ttIncl_t_c_r.length)
              ...
              } // End loop on subbands // End loop on precincts
