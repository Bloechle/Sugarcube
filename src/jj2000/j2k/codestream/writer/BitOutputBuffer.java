...
/**
 * This class implements a buffer for writing bits, with the required bit stuffing policy for the packet headers. The bits are stored in a byte array in the
 * order in which they are written. The byte array is automatically reallocated and enlarged whenever necessary. A BitOutputBuffer object may be reused by
 * calling its 'reset()' method.
 *
 * <P>NOTE: The methods implemented in this class are intended to be used only in writing packet heads, since a special bit stuffing procedure is used, as
...
public class BitOutputBuffer
{
  ...
  /**
   * Resets the buffer. This rewinds the current position to the start of the buffer and sets all tha data to 0. Note that no new buffer is allocated, so this
   * will affect any data that was returned by the 'getBuffer()' method.
     *
   */
  public void reset()
  {
  ...
  /**
   * Writes a bit to the buffer at the current position. The value 'bit' must be either 0 or 1, otherwise it corrupts the bits that have been already written.
   * The buffer is enlarged, by 'SZ_INCR' bytes, if necessary.
   *
  ...
  /**
   * Writes the n least significant bits of 'bits' to the buffer at the current position. The least significant bit is written last. The 32-n most significant
   * bits of 'bits' must be 0, otherwise corruption of the buffer will result. The buffer is enlarged, by 'SZ_INCR' bytes, if necessary.
   *
   * <P>This method is declared final to increase performance.
   *
  ...
  /**
   * Returns the byte buffer. This is the internal byte buffer so it should not be modified. Only the first N elements have valid data, where N is the value
  ...
  /**
   * Returns the byte buffer data in a new array. This is a copy of the internal byte buffer. If 'data' is non-null it is used to return the data. This array
   * should be large enough to contain all the data, otherwise a IndexOutOfBoundsException is thrown by the Java system. The number of elements returned is what
   * 'getLength()' returns.
   *
   * @param data If non-null this array is used to return the data, which mus be large enough. Otherwise a new one is created and returned.
   *
