...
/**
 * This class implements the tag tree encoder. A tag tree codes a 2D matrix of integer elements in an efficient way. The encoding procedure 'encode()' codes
 * information about a value of the matrix, given a threshold. The procedure encodes the sufficient information to identify whether or not the value is greater
 * than or equal to the threshold.
 *
 * <p>The tag tree saves encoded information to a BitOutputBuffer.</p>
 *
 * <p>A particular and useful property of tag trees is that it is possible to change a value of the matrix, provided both new and old values of the element are
 * both greater than or equal to the largest threshold which has yet been supplied to the coding procedure 'encode()'. This property can be exploited through
 * the 'setValue()' method.</p>
 *
 * <p>This class allows saving the state of the tree at any point and restoring it at a later time, by calling save() and restore().</p>
 *
 * <p>A tag tree can also be reused, or restarted, if one of the reset() methods is called.</p>
 *
 * <p>The TagTreeDecoder class implements the tag tree decoder.</p>
 *
 * <p>Tag trees that have one dimension, or both, as 0 are allowed for convenience. Of course no values can be set or coded in such cases.</p>
 *
 * @see BitOutputBuffer
 *
...
public class TagTreeEncoder
{
  ...
  protected int treeS[][];
  /**
   * The saved tag tree values. The first index is the level, starting at level 0 (leafs). The second index is the element within the level, in lexicographical
   * order.
   */
  protected int treeVbak[][];
  /**
   * The saved tag tree state. The first index is the level, starting at level 0 (leafs). The second index is the element within the level, in lexicographical
   * order.
   */
  ...
  /**
   * Creates a tag tree encoder with 'w' elements along the horizontal dimension and 'h' elements along the vertical direction. The total number of elements is
   * thus 'vdim' x 'hdim'.
   *
  ...
  /**
   * Creates a tag tree encoder with 'w' elements along the horizontal dimension and 'h' elements along the vertical direction. The total number of elements is
   * thus 'vdim' x 'hdim'. The values of the leafs in the tag tree are initialized to the values of the 'val' array.
   *
   * <p>The values in the 'val' array are supposed to appear in lexicographical order, starting at index 0.</p>
   *
   * @param h The number of elements along the horizontal direction.
   *
   * @param w The number of elements along the vertical direction.
  ...
  /**
   * Initializes the variables of this class, given the dimensions at the base level (leaf level). All the state ('treeS' array) and values ('treeV' array) are
   * intialized to 0. This method is called by the constructors.
   *
   * @param w The number of elements along the vertical direction.
   *
   * @param h The number of elements along the horizontal direction.
  ...
  /**
   * Changes the value of a leaf in the tag tree. The new and old values of the element must be not smaller than the largest threshold which has yet been
   * supplied to 'encode()'.
   *
  ...
  /**
   * Sets the values of the leafs to the new set of values and updates the tag tree accordingly. No leaf can change its value if either the new or old value is
   * smaller than largest threshold which has yet been supplied to 'encode()'. However such a leaf can keep its old value (i.e. new and old value must be
   * identical.
   *
   * <p>This method is more efficient than the setValue() method if a large proportion of the leafs change their value. Note that for leafs which don't have
   * their value defined yet the value should be Integer.MAX_VALUE (which is the default initialization value).</p>
   *
  ...
  /**
   * Encodes information for the specified element of the tree, given the threshold and sends it to the 'out' stream. The information that is coded is whether
   * or not the value of the element is greater than or equal to the value of the threshold.
   *
   * @param m The vertical index of the element.
   *
   * @param n The horizontal index of the element.
