...
public class FileBitstreamReaderAgent extends BitstreamReaderAgent 
	...
	throws IOException {
        ...
        usePOCQuit = pl.bool("poc_quit");

        // Get decoding rate
        boolean rateInBytes;
        boolean parsing = pl.bool("parsing");
        try {
            trate = pl.real("rate");
            if(trate==-1) {
	...
	// Check that '-rate' and '-nbytes' are not used at the same time
        ...
        if(tnbytes!=defaults.real("nbytes")) {
            rateInBytes = true;
        } else {
            rateInBytes = false;
        }
        ...
        isTruncMode = !pl.bool("parsing");

        // Check if quit conditions are being used
        int ncbQuit;
        try {
             ncbQuit = pl.integer("ncb_quit");
        } catch (NumberFormatException e) {
	...
	int tilePartStart;
		...
		trate = tnbytes*8f/hd.getMaxCompImgWidth()/
            ...
            if(pl.value("res")==null) {
                targetRes = decSpec.dls.getMin();
            } else {
                try {
                    targetRes = pl.integer("res");
                    if(targetRes<0) {
                        throw new
