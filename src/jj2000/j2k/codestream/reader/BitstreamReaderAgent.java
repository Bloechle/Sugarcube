...
/**
 * This is the generic interface for bit stream reader agents. A bit stream reader agent is an entity that allows reading from a bit stream and requesting
 * compressed code-blocks. It can be a simple file reader, or a network connection, or anything else.
 *
 * <p>The bit stream reader agent allows to make request for compressed block data in any order. The amount of data returned would normally depend on the data
 * available at the time of the request, be it from a file or from a network connection.</p>
 *
 * <p>The bit stream reader agent has the notion of a current tile, and coordinates are relative to the current tile, where applicable.</p>
 *
 * <p>Resolution level 0 is the lowest resolution level, i.e. the LL subband alone.</p>
 *
 */
public abstract class BitstreamReaderAgent implements CodedCBlkDataSrcDec
{
  ...
  /**
   * Whether or not the components in the current tile uses a derived quantization step size (only relevant in non reversible quantization mode). This field is
  ...
  /**
   * Dequantization parameters of all subbands and all components in the current tile. The value is actualized by the setTile method in
  ...
  /**
   * The list of parameters that is accepted by the bit stream readers. They start with 'B'.
   */
  private static final String[][] pinfo = null;
  /**
   * The maximum number of decompostion levels for each component of the current tile. It means that component c has mdl[c]+1 resolution levels (indexed from 0
   * to mdl[c])
   *
   */
  protected int mdl[];
  ...
  /**
   * The subband trees for each component in the current tile. Each element in the array is the root element of the subband tree for a component. The number of
   * magnitude bits in each subband (magBits member variable) is not initialized.
   *
   */
  ...
  /**
   * The horizontal offsets of the upper-left corner of the current tile (not active tile) with respect to the canvas origin, in the component hi-res grid, for
   * each component.
   */
  protected final int offX[];
  /**
   * The vertical offsets of the upper-left corner of the current tile (not active tile) with respect to the canvas origin, in the component hi-res grid, for
   * each component.
   */
  protected final int offY[];
  /**
   * The horizontal coordinates of the upper-left corner of the active tile, with respect to the canvas origin, in the component hi-res grid, for each
   * component.
   */
  ...
  /**
   * Initializes members of this class. This constructor takes a HeaderDecoder object. This object must be initialized by the constructor of the implementing
   * class from the header of the bit stream.
   *
   * @param hd The decoded header of the bit stream from where to initialize the values.
   *
  ...
  /**
   * Returns the overall width of the current tile in pixels for the given (tile) resolution level. This is the tile's width without accounting for any
   * component subsampling.
   *
   * <p>Note: Tile resolution level indexes may be different from tile-component resolution index. They are indeed indexed starting from the lowest number of
   * decomposition levels of each component of the tile.</p>
   *
   * <p>For an image (1 tile) with 2 components (component 0 having 2 decomposition levels and component 1 having 3 decomposition levels), the first
   * (tile-)component has 3 resolution levels and the second one has 4 resolution levels, whereas the tile has only 3 resolution levels available.</p>
   *
  ...
  /**
   * Returns the overall width of the image in pixels, for the given (image) resolution level. This is the image's width without accounting for any component
   ...
   *
   * <p>Note: Component resolution level indexes may differ from tile-component resolution index. They are indeed indexed starting from the lowest number of
   * decomposition levels of same component of each tile.</p>
   *
   * <p>Example: For an image (2 tiles) with 1 component (tile 0 having 2 decomposition levels and tile 1 having 3 decomposition levels), the first
   * tile(-component) has 3 resolution levels and the second one has 4 resolution levels, whereas the component has only 3 resolution levels available.</p>
   *
   * @param c The index of the component, from 0 to N-1.
   *
   * @param rl The resolution level, from 0 to L.
   ...
   *
   * <p>Note: this method is not able to return subband tree for a tile different than the current one.</p>
   *
   * <p>The number of magnitude bits ('magBits' member variable) for each subband is not initialized.</p>
   *
   * @param t The tile index
   *
   ...
   *
   * @param pl The parameter list containing parameters applicable to the bit stream read (other parameters may also be present).
   *
   * @param decSpec The decoder specifications
   *
   ...
   *
   * @exception IOException If an I/O error occurs while reading initial data from the bit stream.
   * @exception IllegalArgumentException If an unrecognised bit stream reader option is present.
   *
   */
    ...
    throws IOException
  {
    ...
    // Check parameters
    pl.checkList(BitstreamReaderAgent.OPT_PREFIX, pl.toNameArray(BitstreamReaderAgent.getParameterInfo()));
   ...
   * @param rl The resolution level, from 0 to L.
   *
   * @return the precinct partition width for the specified component, resolution level and tile.
   *
   */
   ...
   * @param rl The resolution level, from 0 to L.
   *
   * @return The precinct partition height in the specified component, for the specified resolution level, for the current tile.
   *
   */
  ...
  /**
   * Initialises subbands fields, such as number of code-blocks, code-blocks dimension and number of magnitude bits, in the subband tree. The nominal code-block
   * width/height depends on the precincts dimensions if used. The way the number of magnitude bits is computed depends on the quantization type (reversible,
   * derived, expounded).
   *
   * @param c The component index
   *
