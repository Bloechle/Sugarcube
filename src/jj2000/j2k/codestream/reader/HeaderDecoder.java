...
import jj2000.icc.ICCProfileException;
...
/**
 * This class reads main and tile-part headers from the codestream given a RandomAccessIO instance located at the beginning of the codestream (i.e just before
 * the SOC marker) or at the beginning of a tile-part (i.e. just before a SOT marker segment) respectively.
 *
 * <p>A marker segment includes a marker and eventually marker segment parameters. It is designed by the three letters code of the marker associated with the
 * marker segment. JPEG 2000 part 1 defines 6 types of markers segments:
 *
 * <ul> <li> Delimiting : SOC, SOT, SOD, EOC</li>
 *
 ...
 *
 * <li> Informational: CRG, COM.</li> </ul></p>
 *
 * <p>The main header is read when the constructor is called whereas tile-part headers are read when the FileBitstreamReaderAgent instance is created. The
 * reading is done in 2 passes:
 *
 * <ol> <li>All marker segments are buffered and their corresponding flag is activated (extractMainMarkSeg and extractTilePartMarkSeg methods).</li>
 *
 * <li>Buffered marker segment are analyzed in a logical way and specifications are stored in appropriate member of DecoderSpecs instance (readFoundMainMarkSeg
 * and readFoundTilePartMarkSeg methods).</li> </ol></p>
 *
 * <p>Whenever a marker segment is not recognized a warning message is displayed and its length parameter is used to skip it.</p>
 *
 * <p>The information found in this header is stored in HeaderInfo and DecoderSpecs instances.</p>
 *
 * @see DecoderSpecs
 * @see HeaderInfo
 * @see Decoder
...
public class HeaderDecoder implements ProgressionType, Markers, StdEntropyCoderOptions
{
   ...
   *
   * @return The coordinate of the tiling origin, in the canvas system, on the reference grid.
   *
  ...
  /**
   * Returns true if the original data of the specified component was signed. If the data was not signed a level shift has to be applied at the end of the
   * decompression chain.
   *
   ...
   *
   * @param rb The number of range bits for each component. Must be the number of range bits of the mixed components.
   *
   ...
   *
   * @return The precinct partition width for the specified tile-component and resolution level
   *
   */
   ...
   * @param rl the resolution level
   *
   * @return The precinct partition height for the specified tile-component and resolution level
   *
   */
   ...
   *
   * @param filtIdx Int array of one element to return the type of the wavelet filter.
   *
   */
   ...
   *
   * <p>SIZ is a fixed information marker segment containing informations about image and tile sizes. It is required in the main header immediately after
   * SOC.</p>
   *
   * @param ehs The encoded header stream
   *
   * @exception IOException If an I/O error occurs while reading from the encoded header stream
   *
   */
  ...
  /**
   * Reads a CRG marker segment and checks its length. CRG is an informational marker segment that allows specific registration of components with respect to
   * each other.
   *
   * @param ehs The encoded header stream
   *
  ...
  /**
   * Reads a COM marker segments and realigns the bit stream at the point where the next marker segment should be found. COM is an informational marker segment
   * that allows to include unstructured data in the main and tile-part headers.
   *
   * @param ehs The encoded header stream
   *
   * @param mainh Flag indicating whether or not this marker segment is read from the main header.
   *
  ...
  /**
   * Reads a QCD marker segment and realigns the codestream at the point where the next marker should be found. QCD is a functional marker segment that
  ...
  /**
   * Reads a QCC marker segment and realigns the codestream at the point where the next marker should be found. QCC is a functional marker segment that
   ...
   * @param tpIdx Tile-part index
   *
   * @exception IOException If an I/O error occurs while reading from the encoder header stream
   *
   */
    ...
    int tpIdx) throws IOException
  {
    ...
    if ((ecOptions
      ...
      throw new CorruptedCodestreamException("Unknown \"code-block "
  ...
  /**
   * Reads TLM marker segment and realigns the codestream where the next marker should be found. Informations stored in these fields are currently NOT taken
   * into account.
   *
   * @param ehs The encoder header stream.
   *
  ...
  /**
   * Reads PLM marker segment and realigns the codestream where the next marker should be found. Informations stored in these fields are currently not taken
   * into account.
   *
   * @param ehs The encoder header stream.
   *
  ...
  /**
   * Reads the PLT fields and realigns the codestream where the next marker should be found. Informations stored in these fields are currently NOT taken into
   * account.
   *
   * @param ehs The encoder header stream.
   *
   ...
   *
   * <p>May be used in tile or main header. If used in main header, it refers to the maxshift value of a component in all tiles. When used in tile header, only
   * the particular tile-component is affected.</p>
   *
   * @param ehs The encoder header stream.
   *
   * @param mainh Flag indicating whether or not this marker segment is read from the main header.
  ...
  /**
   * This method extract a marker segment from the main header and stores it into a byte buffer for the second pass. The marker segment is first identified.
   * Then its flag is activated. Finally, its content is buffered into a byte array stored in an hashTable.
   *
   * <p>If the marker is not recognized, it prints a warning and skips it according to its length.</p>
   *
    ...
    switch (marker)
    {
      case SIZ:
        if ((nfMarkSeg & SIZ_FOUND) != 0)
          throw new CorruptedCodestreamException("More than one SIZ marker "
  ...
  /**
   * This method extracts a marker segment in a tile-part header and stores it into a byte buffer for the second pass. The marker is first recognized, then its
   * flag is activated and, finally, its content is buffered in an element of byte arrays accessible thanks to a hashTable. If a marker segment is not
   * recognized, it prints a warning and skip it according to its length.
   *
  ...
  /**
   * Creates a HeaderDecoder instance and read in two passes the main header of the codestream. The first and last marker segments shall be respectively SOC and
   * SOT.
   *
   ...
   *
   * @exception IOException If an I/O error occurs while reading from the encoded header stream.
   * @exception EOFException If the end of the encoded header stream is reached before getting all the data.
   * @exception CorruptedCodestreamException If invalid data is found in the codestream main header.
   *
   */
  ...
  /**
   * Creates and returns the entropy decoder corresponding to the information read from the codestream header and with the special additional parameters from
   * the parameter list.
   *
   * @param src The bit stream reader agent where to get code-block data from.
   *
   * @param pl The parameter list containing parameters applicable to the entropy decoder (other parameters can also be present).
   *
  ...
  /**
   * Creates and returns the PalettizedColorSpaceMapper which uses the input samples as indicies into a sugarcube.app.sample palette to construct the output.
   *
   * @param src The bit stream reader agent where to get code-block data from.
   * @param csMap provides color space information from the image file
   *
  ...
  /**
   * Creates and returns the Resampler which converts the input source to one in which all channels have the same number of samples. This is required for
  ...
  /**
   * Creates and returns the ROIDeScaler corresponding to the information read from the codestream header and with the special additional parameters from the
   * parameter list.
   *
   * @param src The bit stream reader agent where to get code-block data from.
   *
  ...
  /**
   * Returns the parameters that are used in this class. It returns a 2D String array. Each of the 1D arrays is for a different option, and they have 3
   * elements. The first element is the option name, the second one is the synopsis and the third one is a long description of what the parameter is. The
   * synopsis or description may be 'null', in which case it is assumed that there is no synopsis or description of the option, respectively.
   *
   ...
   *
   * @return An input stream containing the packed packet headers for a particular tile
   *
   * @exception IOException If an I/O error occurs while reading from the encoder header stream
   *
   */
