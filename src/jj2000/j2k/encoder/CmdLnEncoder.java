...
/**
 * This class runs JJ2000's encoder from the command line interface. It parses command-line arguments to fill a ParameterList object which will be provided to
 * an Encoder object.
 *
 */
public class CmdLnEncoder
{
  ...
  /**
   * Instantiates a command line encoder object, with the 'argv' command line arguments. It also initializes the default parameters. If the argument list is
   * empty an IllegalArgumentException is thrown. If an error occurs while parsing the arguments error messages are written to stderr and the run exit code is
   * set to non-zero, see getExitCode()
   *
  ...
  public CmdLnEncoder(String argv[])
  {
    ...
    if (pl.value("pfile") != null)
    {
      ...
      try
      {
        is = new FileInputStream(pl.value("pfile"));
