...
/**
 * This class is the main class of JJ2000's encoder. It instantiates all objects of the chain and launchs the encoding process. It then writes the header and
 * the compressed bit stream to the output file. Finally, packed packet headers (through codestream post-manipulation) and file-format may be created if needed.
 *
 * <p>First the encoder should be initialized with a ParameterList object provided through the constructor. Then, the run() method is invoked and the encoder
 ...
 * indicates that an error has occurred.</p>
 *
 * <p>The modules are inserted in the encoding chain with the following order:</p>
 *
 * <ul> <li>ImgReader</li> <li>ImgDataJoiner (if multiple image readers)</li> <li>Tiler</li> <li>ForwCompTransf</li> <li>ImgDataConverter</li>
 * <li>ForwardWT</li> <li>Quantizer</li> <li>ROIScaler</li> <li>EntropyCoder</li> <li>PostCompRateAllocator</li> </ul>
 *
 * <p>The encoder uses a pull model. This means that the last module (PostCompRateAllocator) requests data from its source (EntropyCoder), ...</p>
 *
 * <p>Writing of the codestream writing (header+bit stream) is realized by HeaderEncoder and CodestreamWriter modules.</p>
 *
 * <p>Packed packet headers and file-format creation are carried out by CodestreamManipulator and FileFormatWriter modules respectively.</p>
 *
 * <p>Many modules of the encoder may behave differently depending on the tile-component. The specifications of their behaviour are kept in specialized modules
 * extending ModuleSpec class. All these modules are accessible through an instance of EncoderSpecs class.</p>
 *
 * @see ImgReader
 * @see ImgDataJoiner
 * @see ForwCompTransf
...
public class Encoder implements Runnable
{
   ...
   *
   * @param pl The ParameterList for this decoder (contains also defaults values);
     *
   */
  ...
  public void run()
  {
    ...
    try
    {
      ...
      if (pl.value("file_format").equals("on"))
      {
        useFileFormat = true;
        if (pl.value("rate") != null
          && pl.real("rate")
          != defpl.real("rate"))
      ...
      if (useFileFormat)
      {
        ...
        if (outext == null || !outext.equalsIgnoreCase(".jp2"))
          if (!pl.bool("disable_jp2_extension"))
          {
      ...
      if (pl.value("tiles") == null)
      {
        error("No tiles option specified", 2);
        return;
      }

      if (pl.value("pph_tile").equals("on"))
      {
        pphTile = true;

        if (pl.value("Psop").equals("off"))
        {
          pl.put("Psop", "on");
          tempSop = true;
        }
        if (pl.value("Peph").equals("off"))
        {
          pl.put("Peph", "on");
          tempEph = true;
        }
      ...
      if (pl.value("pph_main").equals("on"))
      {
        pphMain = true;
      ...
      try
      {
        pktspertp = pl.integer("tile_parts");
        if (pktspertp != 0)
        {
          if (pl.value("Psop").equals("off"))
          {
      ...
      // **** ImgReader ****
      sgtok = new StringTokenizer(pl.value("i"), ",");
      ncomp = 0;
      ppminput = false;
      ...
      stok = new StreamTokenizer(new StringReader(pl.value("tiles")));
      stok.eolIsSignificant(false);

      stok.nextToken();
      if (stok.ttype != StreamTokenizer.TT_NUMBER)
      {
        error("An error occurred while parsing the tiles option: " + pl.value("tiles"), 2);
        return;
      }
      tw = (int) stok.nval;
      stok.nextToken();
      ...
      sgtok = new StringTokenizer(pl.value("ref"));
      try
      {
      ...
      sgtok = new StringTokenizer(pl.value("tref"));
      try
      {
      ...
      {
        throw new IllegalArgumentException("Error while parsing 'tref' "
          + "option");
      }
      catch (NumberFormatException e)
      {
      ...
      if (ppminput && pl.value("Mct") != null
        && pl.value("Mct").equals("off"))
        FacilityManager.getMsgLogger().
      ...
      if (pktspertp > 0 || pphTile || pphMain)
      {
        ...
        try
        {
          CodestreamManipulator cm = new CodestreamManipulator(outname, ntiles, pktspertp,
      ...
      if (verbose)
      {
        ...
        if (pl.value("Rroi") != null && !useFileFormat
          && pl.integer("tile_parts") == 0)
        {
   ...
   * and they have 4 elements. The first element is the option name, the second one is the synopsis, the third one is a long description of what the parameter
  ...
  /**
   * Returns all the parameters used in the encoding chain. It calls parameter from each module and store them in one array (one row per parameter and 4
   * columns).
   *
  ...
  /**
   * Prints the error message 'msg' to standard err, prepending "ERROR" to it, and sets the exitCode to 'code'. An exit code different than 0 indicates that
   * there where problems.
   *
  ...
  /**
   * Prints the parameters in 'pinfo' to the provided output, 'out', showing the existing defaults. The 'pinfo' argument is a 2D String array. The first
   * dimension contains String arrays, 1 for each parameter. Each of these arrays has 3 elements, the first element is the parameter name, the second element is
   * the synopsis for the parameter and the third one is a long description of the parameter. If the synopsis or description is 'null' then no synopsis or
   * description is printed, respectively. If there is a default value for a parameter it is also printed.
   *
