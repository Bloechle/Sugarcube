...
/**
 * This abstract class represents a subband in a bidirectional tree structure that describes the subband decomposition for a wavelet transform. This class is
 * implemented by the SubbandAn and SubbandSyn classes, which are for the analysis and synthesis sides respectively.
 *
 * <p>The element can be either a node or a leaf of the tree. If it is a node, it has 4 descendants (LL, HL, LH and HH). If it is a leaf, it has no
 * descendant.</p>
 *
 ...
 * The only exception is the root element which, for obvious reasons, has no parent (i.e. it is null).</p>
 *
...
public abstract class Subband
{
  ...
  /**
   * The level in the tree to which this subband belongs, which is the number of performed wavelet decompositions to get this subband. It is 0 for the top-level
   * (i.e. root) node.
   */
  public int level;
  /**
  ...
  /**
   * The base 2 exponent of the analysis gain of the subband. The analysis gain of a subband is defined as the gain of the previous subband (i.e. the one from
   * which this one was obtained) multiplied by the line gain and by the column gain. The line (column) gain is the gain of the line (column) filter that was
   * used to obtain it, which is the DC gain for a low-pass filter and the Nyquist gain for a high-pass filter. It is 0 by default.
   *
   * <p>Using the base 2 exponent of the value contrains the possible gains to powers of 2. However this is perfectly compatible to the filter normalization
   * policy assumed here. See the split() method for more details.</p>
   *
   * @see #split
     *
   */
  ...
  /**
   * The subband index within its resolution level. This value uniquely identifies a subband within a resolution level and a decomposition level within it. Note
   * that only leaf elements represent "real" subbands, while node elements represent only intermediate stages.
   *
   * <p>It is defined recursively. The root node gets a value of 0. For a given node, with a subband index 'b', its LL descendant gets 4*b, its HL descendant
   * 4*b+1, its LH descendant 4*b+2, and its HH descendant 4*b+3, for their subband indexes.</p>
     *
   */
  public int sbandIdx = 0;
  /**
   * The horizontal coordinate of the upper-left corner of the subband, with respect to the canvas origin, in the component's grid and subband's decomposition
   * level. This is the real horizontal index of the first column of this subband. If even the horizontal decomposition of this subband should be done with the
   * low-pass-first convention. If odd it should be done with the high-pass-first convention.
     *
   */
  public int ulcx;
  /**
   * The vertical coordinate of the upper-left corner of the subband, with respect to the canvas origin, in the component's grid and subband's decomposition
   * level. This is the real vertical index of the first column of this subband. If even the vertical decomposition of this subband should be done with the
   * low-pass-first convention. If odd it should be done with the high-pass-first convention.
     *
   */
  public int ulcy;
   ...
   *
   * @param vfilter The vertical wavelet filter used to decompose this subband.
   *
  ...
  /**
   * Initializes the childs of this node with the correct values. The sizes of the child subbands are calculated by taking into account the position of the
   * subband in the canvas.
   *
   * <p>For the analysis subband gain calculation it is assumed that analysis filters are normalized with a DC gain of 1 and a Nyquist gain of 2.</p>
     *
   */
  ...
  /**
   * Creates a Subband element with all the default values. The dimensions are (0,0), the upper left corner is (0,0) and the upper-left corner with respect to
   * the canvas is (0,0) too.
     *
   */
  public Subband()
  {
   ...
   * <p>For the analysis subband gain calculation it is assumed that analysis filters are normalized with a DC gain of 1 and a Nyquist gain of 2.</p>
   *
   * <p>This constructor does not initialize the value of the magBits member variable. This variable is normally initialized by the quantizer, on the encoder
   * side, or the bit stream reader, on the decoder side.</p>
   *
   ...
   *
   * @param hfilters The horizontal wavelet filters (analysis or synthesis) for each resolution level, starting at resolution level 0. If there are less
   * elements in the array than there are resolution levels, the last element is used for the remaining resolution levels.
   *
   * @param vfilters The vertical wavelet filters (analysis or synthesis) for each resolution level, starting at resolution level 0. If there are less elements
   * in the array than there are resolution levels, the last element is used for the remaining resolution levels.
   *
  ...
  /**
   * Returns the next subband in the same resolution level, following the subband index order. If already at the last subband then null is returned. If this
   * subband is not a leaf an IllegalArgumentException is thrown.
   *
   ...
   *
   * @return The first leaf element in the next higher resolution level, or null if there is no higher resolution level.
     *
   */
  ...
  /**
   * Returns a reference to the Subband element to which the specified point belongs. The specified point must be inside this (i.e. the one defined by this
   * object) subband. This method searches through the tree.
   *
