 ...
 *
 * <p>The image can be reconstructed at different (image) resolution levels indexed from the lowest resolution available for each tile-component. This is
 * controlled by the setImgResLevel() method.</p>
 *
 ...
 *
 * <p>This implementation does not support progressive data: Data is considered to be non-progressive (i.e. "final" data) and the 'progressive' attribute of the
 * 'DataBlk' class is always set to false, see the 'DataBlk' class.</p>
 *
 * @see DataBlk
 *
 */
public class InvWTFull extends InverseWT
{
  ...
  private int[] ndl;
  /**
   * The reversible flag for each component in each tile. The first index is the tile index, the second one is the component index. The reversibility of the
   * components for each tile are calculated on a as needed basis.
     *
   */
  ...
  /**
   * Returns the reversibility of the current subband. It computes iteratively the reversibility of the child subbands. For each subband it tests the
   * reversibility of the horizontal and vertical synthesis filters used to reconstruct this subband.
   *
   * @param subband The current subband.
   *
   * @return true if all the filters used to reconstruct the current subband are reversible
     *
   */
  ...
  /**
   * Returns the reversibility of the wavelet transform for the specified component, in the current tile. A wavelet transform is reversible when it is suitable
   ...
   *
   * <p>The returned value corresponds to the nominal dynamic range of the reconstructed image data, as long as the getNomRangeBits() method of the source
   * returns a value corresponding to the nominal dynamic range of the image data and not not of the wavelet coefficients.</p>
   *
   ...
   *
   * <p>This default implementation assumes that the wavelet transform does not modify the fixed point. If that were the case this method should be
   * overriden.</p>
   *
   * @param c The index of the component.
   *
  ...
  /**
   * Returns a block of image data containing the specifed rectangular area, in the specified component, as a reference to the internal buffer (see below). The
   * rectangular area is specified by the coordinates and dimensions of the 'blk' object.
   *
   * <p>The area to return is specified by the 'ulx', 'uly', 'w' and 'h' members of the 'blk' argument. These members are not modified by this method.</p>
   *
   ...
   *
   * <p>The returned data has its 'progressive' attribute unset (i.e. false).</p>
   *
  ...
  /**
   * Returns a block of image data containing the specifed rectangular area, in the specified component, as a copy (see below). The rectangular area is
   * specified by the coordinates and dimensions of the 'blk' object.
   *
   * <p>The area to return is specified by the 'ulx', 'uly', 'w' and 'h' members of the 'blk' argument. These members are not modified by this method.</p>
   *
   ...
   * after being returned. The 'offset' of the returned data is 0, and the 'scanw' is the same as the block's width. See the 'DataBlk' class.</p>
   *
   * <p>If the data array in 'blk' is <tt>null</tt>, then a new one is created. If the data array is not <tt>null</tt> then it must be big enough to contain the
   * requested area.</p>
   *
   * <p>The returned data always has its 'progressive' attribute unset (i.e false)</p>
   *
   * @param blk Its coordinates and dimensions specify the area to return. If it contains a non-null data array, then it must be large enough. If it contains a
   * null data array a new one is created. The fields in this object are modified to return the data.
   *
   * @param c The index of the component from which to get the data.
   *
   * @return The requested DataBlk
  ...
  /**
   * Performs the 2D inverse wavelet transform on a subband of the image, on the specified component. This method will successively perform 1D filtering steps
   * on all columns and then all lines of the subband.
   *
  ...
  /**
   * Performs the inverse wavelet transform on the whole component. It iteratively reconstructs the subbands from leaves up to the root node. This method is
   * recursive, the first call to it the 'sb' must be the root of the subband tree. The method will then process the entire subband tree by calling itslef
   * recursively.
   *
