...
public class ForwWTFull extends ForwardWT
{
  ...
  /**
   * The subband trees of each tile-component. The array is allocated by the constructor of this class and updated by the getAnSubbandTree() method when needed.
   * The first index is the tile index (in lexicographical order) and the second index is the component index.
   *
   * <p>The subband tree for a component in the current tile is created on the first call to getAnSubbandTree() for that component, in the current tile. Before
   * that, the element in 'subbTrees' is null.</p>
     *
   */
  ...
  /**
   * The horizontal index of the last "sent" code-block in the current subband in each component. It should be -1 if none have been sent yet.
     *
   */
  private int lastn[];
  /**
   * The vertical index of the last "sent" code-block in the current subband in each component. It should be 0 if none have been sent yet.
     *
   */
  private int lastm[];
  /**
  ...
  /**
   * Cache object to avoid excessive allocation/desallocation. This variable makes the class inheritently thread unsafe.
   */
  Coord ncblks;
   ...
   *
   * @param cb0x The horizontal coordinate of the code-block partition origin on the reference grid.
   *
   * @param cb0y The vertical coordinate of the code-block partition origin on the reference grid.
   *
   * @see ForwardWT
     *
   */
  ...
  /**
   * Returns the horizontal analysis wavelet filters used in each level, for the specified component and tile. The first element in the array is the filter used
   * to obtain the lowest resolution (resolution level 0) subbands (i.e. lowest frequency LL subband), the second element is the one used to generate the
  ...
  /**
   * Returns the vertical analysis wavelet filters used in each level, for the specified component and tile. The first element in the array is the filter used
   * to obtain the lowest resolution (resolution level 0) subbands (i.e. lowest frequency LL subband), the second element is the one used to generate the
   * resolution level 1 subbands, and so on. If there are less elements in the array than the number of resolution levels, then the last one is assumed to
   * repeat itself.
   *
   ...
   *
   * <p>The data returned by this method is the data in the internal buffer of this object, and thus can not be modified by the caller. The 'offset' and 'scanw'
   * of the returned data have, in general, some non-zero value. The 'magbits' of the returned data is not set by this method and should be ignored. See the
   * 'CBlkWTData' class.</p>
   *
   ...
   *
   * @param cblk If non-null this object will be used to return the new code-block. If null a new one will be allocated and returned.
   *
   ...
   * called before for the new current tile.</p>
   *
   * <p>The data returned by this method is always a copy of the internal data of this object, and it can be modified "in place" without any problems after
   * being returned. The 'offset' of the returned data is 0, and the 'scanw' is the same as the code-block width. The 'magbits' of the returned data is not set
   * by this method and should be ignored. See the 'CBlkWTData' class.</p>
   *
   * <p>The 'ulx' and 'uly' members of the returned 'CBlkWTData' object contain the coordinates of the top-left corner of the block, with respect to the tile,
   * not the subband.</p>
   *
  ...
  /**
   * Return the data type of this CBlkWTDataSrc. Its value should be either DataBlk.TYPE_INT or DataBlk.TYPE_FLOAT but can change according to the current
   * tile-component.
   *
   ...
   *
   * @return Its returns the next subband that will be used to get the next code-block to return by the getNext[Intern]CodeBlock method.
     *
   */
   ...
   *
   * @param subband The structure containing the coordinates of the current subband in the whole band to decompose.
   *
  ...
  /**
   * Performs the 2D forward wavelet transform on a subband of the initial band. This method will successively perform 1D filtering steps on all lines and then
   * all columns of the subband. In this class only filters with floating point implementations can be used.
   *
   * @param band The band containing the float data to decompose
   *
   * @param subband The structure containing the coordinates of the subband in the whole band to decompose.
   *
   * @param c The index of the current component to decompose
     *
   */
   ...
   *
   * <p>This method resets the 'subbTrees' array, and recalculates the values of the 'reversible' array. It also resets the decomposed component buffers.</p>
   *
  ...
  /**
   * Initialises subbands fields, such as number of code-blocks and code-blocks dimension, in the subband tree. The nominal code-block width/height depends on
   * the precincts dimensions if used.
   *
