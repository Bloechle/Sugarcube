...
public class ArbROIMaskGenerator extends ROIMaskGenerator
{
   ...
   *
   * <P> In order to get the mask for a particular Subband, the subband tree is traversed and at each decomposition, the ROI masks are computed.
   *
   * <P> The widths of the synthesis filters corresponding to the wavelet filters used in the wavelet transform are used to expand the ROI masks in the
   * decompositions.
   *
   ...
   *
   * <P> Once the mask is generated in the pixel domain. it is decomposed following the same decomposition scheme as the wavelet transform.
   *
  ...
  /**
   * This function decomposes the mask for a node in the subband tree. after the mask is decomposed for a node, this function is called for the children of the
   * subband. The decomposition is done line by line and column by column
   *
