...
/**
 * This class reads the file format wrapper that may or may not exist around a valid JPEG 2000 codestream. Since no information from the file format is used in
 * the actual decoding, this class simply goes through the file and finds the first valid codestream.
 *
...
public class FileFormatReader implements FileFormatBoxes
{
  ...
  /**
   * This method checks whether the given RandomAccessIO is a valid JP2 file and if so finds the first codestream in the file. Currently, the information in the
   * codestream is not used
   *
   * @param in The RandomAccessIO from which to read the file format
   *
      ...
      while (!lastBoxFound)
      {
        ...
        switch (box)
        {
          ...
          case UUID_INFO_BOX:
            ...
            break;
          case READER_REQUIREMENTS_BOX:
            break;
          default:
            FacilityManager.getMsgLogger().printmsg(MsgLogger.WARNING, "Unknown box-type: 0x" + Integer.toHexString(box));
        }
        if (!lastBoxFound)
          in.seek(pos + length);
      }
