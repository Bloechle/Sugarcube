...
/**
 * This class apply component transformations to the tiles depending on user specifications. These transformations can be used to improve compression efficiency
 * but are not related to colour transforms used to map colour values for display purposes. JPEG 2000 part I defines 2 component transformations: RCT
 * (Reversible Component Transformation) and ICT (Irreversible Component Transformation).
 *
 * @see ModuleSpec
 *
 */
public class ForwCompTransf extends ImgDataAdapter implements BlkImgDataSrc
{
   ...
   *
   * <p>This default implementation assumes that the number of fractional bits is not modified by the component mixer.</p>
   *
   * @param c The index of the component.
   *
   * @return The value of the fixed point position of the source since the color transform does not affect it.
     *
   */
   ...
   *
   * @param tdepth If not null the results are stored in this array, otherwise a new array is allocated and returned.
   *
  ...
  /**
   * Returns a string with a descriptive text of which forward component transformation is used. This can be either "Forward RCT" or "Forward ICT" or "No
   * component transformation" depending on the current tile.
   *
  ...
  /**
   * Returns the number of bits, referred to as the "range bits", corresponding to the nominal range of the data in the specified component and in the current
   * tile. If this number is <i>b</i> then for unsigned data the nominal range is between 0 and 2^b-1, and for signed data it is between -2^(b-1) and 2^(b-1)-1.
   * Note that this value can be affected by the multiple component transform.
   *
   * @param c The index of the component.
   *
  ...
  /**
   * Returns true if this transform is reversible in current tile. Reversible component transformations are those which operation can be completely reversed
   * without any loss of information (not even due to rounding).
   *
  ...
  /**
   * Apply forward component transformation associated with the current tile. If no component transformation has been requested by the user, data are not
   * modified.
   *
   * <p>This method calls the getInternCompData() method, but respects the definitions of the getCompData() method defined in the BlkImgDataSrc interface.</p>
   *
   * @param blk Determines the rectangular area to return, and the data is returned in this object.
   *
  ...
  /**
   * Apply the component transformation associated with the current tile. If no component transformation has been requested by the user, data are not modified.
   * Else, appropriate method is called (forwRCT or forwICT).
   *
   * @see #forwRCT
   *
   * @see #forwICT
  ...
  /**
   * Apply forward component transformation to obtain requested component from specified block of data. Whatever the type of requested DataBlk, it always
   * returns a DataBlkInt.
   *
        ...
        case 0: //RGB to Yr conversion
          for (i = h - 1; i >= 0; i--)
          {
            for (mink = k - w; k > mink; k--, k0--, k1--, k2--)
              ...
              // and rounding
              outdata[k] =
                (data0[k] + 2 * data1[k] + bdata[k]) >> 2; // Same as / 4
  ...
  /**
   * Apply forward irreversible component transformation to obtain requested component from specified block of data. Whatever the type of requested DataBlk, it
   * always returns a DataBlkFloat.
   *
   * @param blk Determine the rectangular area to return
   *
   * @param c The index of the requested component
   ...
   *
   * <p>This default implementation changes the tile in the source and re-initializes properly component transformation variables..</p>
   *
  ...
  /**
   * Goes to the next tile, in standard scan-line order (by rows then by columns). An NoNextElementException is thrown if the current tile is the last one (i.e.
   * there is no next tile).
   *
   * <p>This default implementation just advances to the next tile in the source and re-initializes properly component transformation variables.</p>
     *
   */
