 ...
 * Class: ImgReader
 *
 * Description: Generic interface for image readers (from file or other resource)
 *
 *
 *
 * COPYRIGHT:
 ...
 *
 * <p>An ImgReader behaves as an ImgData object. Whenever image data is requested through the getInternCompData() or getCompData() methods, the image data will
 * be read (if it is not buffered) and returned. Implementing classes should not buffer large amounts of data, so as to reduce memory usage.</p>
 *
 * <p>This class sets the image origin to (0,0). All default implementations of the methods assume this.</p>
 *
 * <p>This class provides default implementations of many methods. These default implementations assume that there is no tiling (i.e., the only tile is the
 * entire image), that the image origin is (0,0) in the canvas system and that there is no component subsampling (all components are the same size), but they
 * can be overloaded by the implementating class if need be.</p>
 *
 */
public abstract class ImgReader implements BlkImgDataSrc
{
  ...
  /**
   * Returns the width of the current tile in pixels, assuming there is no-tiling. Since no-tiling is assumed this is the same as the width of the image. The
  ...
  /**
   * Returns the overall height of the current tile in pixels, assuming there is no-tiling. Since no-tiling is assumed this is the same as the width of the
   * image. The value of <tt>h</tt> is returned.
   *
  ...
  /**
   * Returns the number of components in the image. The value of <tt>nc</tt> is returned.
   *
  ...
  /**
   * Returns the width in pixels of the specified tile-component. This default implementation assumes no tiling and no component subsampling (i.e., all
  ...
  /**
   * Returns the height in pixels of the specified tile-component. This default implementation assumes no tiling and no component subsampling (i.e., all
   * components, or components, have the same dimensions in pixels).
   *
  ...
  /**
   * Returns the width in pixels of the specified component in the overall image. This default implementation assumes no component, or component, subsampling
   * (i.e. all components have the same dimensions in pixels).
   *
   * @param c The index of the component, from 0 to C-1.
   *
  ...
  /**
   * Returns the height in pixels of the specified component in the overall image. This default implementation assumes no component, or component, subsampling
   * (i.e. all components have the same dimensions in pixels).
   *
   * @param c The index of the component, from 0 to C-1.
   *
  ...
  /**
   * Changes the current tile, given the new coordinates. An IllegalArgumentException is thrown if the coordinates do not correspond to a valid tile. This
   * default implementation assumes no tiling so the only valid arguments are x=0, y=0.
   *
