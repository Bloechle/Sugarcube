...
/**
 * This class extends the ImgReader abstract class for reading PGX files. PGX is a custom monochrome file format invented specifically to simplify the use of
 * JPEG 2000 with images of different bit-depths in the range 1 to 31 bits per pixel.
 *
 ...
 *
 * <p> If the data is unisigned, level shifting is applied subtracting 2^(bitdepth - 1)</p>
 *
 * <p>Since it is not possible to know the input file byte-ordering before reading its header, this class can not be construct from a RandomAccessIO. So, the
 * constructor has to open first the input file, to read only its header, and then it can create the appropriate BufferedRandomAccessFile (Big-Endian or
 * Little-Endian byte-ordering).</p>
 *
 * <p>NOTE: This class is not thread safe, for reasons of internal buffering.</p>
 *
...
public class ImgReaderPGX extends ImgReader implements EndianType
{
  ...
  /**
   * Returns the number of bits corresponding to the nominal range of the data in the specified component. This is the value of bitDepth which is read in the
   * PGX file header.
   *
   * <P>If this number is <i>b</b> then the nominal range is between -2^(b-1) and 2^(b-1)-1, for originally signed or unsigned data (unsigned data is level
   ...
   * are not modified by this method. The 'offset' and 'scanw' of the returned data can be arbitrary. See the 'DataBlk' class.</p>
   *
   ...
   * considered to contain the returned data, a new array may have been created. Instead, get the array from <tt>blk</tt> after the method has returned.</p>
   *
   ...
   * depends on the action that has been registered in JJ2KExceptionHandler. See JJ2KExceptionHandler for details.</p>
   *
   ...
   *
   * <p>The rectangular area to return is specified by the 'ulx', 'uly', 'w' and 'h' members of the 'blk' argument, relative to the current tile. These members
   * are not modified by this method. The 'offset' of the returned data is 0, and the 'scanw' is the same as the block's width. See the 'DataBlk' class.</p>
   *
   ...
   * contain the block's data. Otherwise an 'ArrayStoreException' or an 'IndexOutOfBoundsException' is thrown by the Java system.</p>
   *
