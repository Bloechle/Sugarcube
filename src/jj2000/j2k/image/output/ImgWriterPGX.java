...
/**
 * This class extends the ImgWriter abstract class for writing PGX files. PGX is a custom monochrome file format invented specifically to simplify the use of
 ...
 *
 * <p> <u>Header:</u> "PG"+ <i>ws</i> +&lt;<i>endianess</i>&gt;+ <i>ws</i> +[<i>sign</i>]+<i>ws</i> + &lt;<i>bit-depth</i>&gt;+" "+&lt;<i>width</i>&gt;+"
 * "+&lt;<i>height</i>&gt;+'\n'</p>
 *
 * <p>where:<br> <ul> <li><i>ws</i> (white-spaces) is any combination of characters ' ' and '\t'.</li> <li><i>endianess</i> equals "LM" or "ML"(resp.
 * little-endian or big-endian)</li> <li><i>sign</i> equals "+" or "-" (resp. unsigned or signed). If omited, values are supposed to be unsigned.</li>
 * <li><i>bit-depth</i> that can be any number between 1 and 31.</li> <li><i>width</i> and <i>height</i> are the image dimensions (in pixels).</li> </ul>
 *
 * <u>Data:</u> The image binary values appear one after the other (in raster order) immediately after the last header character ('\n') and are byte-aligned
 * (they are packed into 1,2 or 4 bytes per sugarcube.app.sample, depending upon the bit-depth value). </p>
 *
 * <p> If the data is unsigned, level shifting is applied adding 2^(bit depth - 1)</p>
 *
 * <p><u>NOTE</u>: This class is not thread safe, for reasons of internal buffering.</p>
 *
 * @see ImgWriter
 *
 * @see BlkImgDataSrc
...
public class ImgWriterPGX extends ImgWriter
{
  ...
  private int c;
  /**
   * The pack length of one sugarcube.app.sample (in bytes, according to the output bit-depth
   */
   ...
   *
   * <p>The size of the image that is written to the file is the size of the component from which to get the data, specified by b, not the size of the source
   * image (they differ if there is some sub-sampling).</p>
   *
   * <p>All the header informations are given by the BlkImgDataSrc source (component width, component height, bit-depth) and sign flag, which are provided to
   * the constructor. The endianness is always big-endian (MSB first).</p>
   *
   ...
   *
   * @param isSigned Whether the datas are signed or not (needed only when writing header).
   *
   * @see DataBlk
     *
   */
   ...
   * image (they differ if there is some sub-sampling).</p>
   *
   * <p>All header information is given by the BlkImgDataSrc source (component width, component height, bit-depth) and sign flag, which are provided to the
   * constructor. The endianness is always big-endian (MSB first).
   *
  ...
  /**
   * Closes the underlying file or netwrok connection to where the data is written. Any call to other methods of the class become illegal after a call to this
   * one.
   *
  ...
  /**
   * Writes the data of the specified area to the file, coordinates are relative to the current tile of the source. Before writing, the coefficients are limited
   * to the nominal range and packed into 1,2 or 4 bytes (according to the bit-depth).
   *
   ...
   *
   * <p>If the data returned from the BlkImgDataSrc source is progressive, then it is requested over and over until it is not progressive anymore.</p>
   *
  ...
  /**
   * Writes the source's current tile to the output. The requests of data issued to the source BlkImgDataSrc object are done by strips, in order to reduce
   * memory usage.
   *
   * <p>If the data returned from the BlkImgDataSrc source is progressive, then it is requested over and over until it is not progressive anymore.</p>
   *
  ...
  /**
   * Returns a string of information about the object, more than 1 line long. The information string includes information from the underlying RandomAccessFile
