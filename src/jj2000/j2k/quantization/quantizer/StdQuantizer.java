...
/**
 * This class implements scalar quantization of integer or floating-point valued source data. The source data is the wavelet transformed image data and the
 * output is the quantized wavelet coefficients represented in sign-magnitude (see below).
 *
...
public class StdQuantizer extends Quantizer
{
  ...
  /**
   * Initializes the source of wavelet transform coefficients. The constructor takes information on whether the quantizer is in reversible, derived or expounded
   * mode. If the quantizer is reversible the value of 'derived' is ignored. If the source data is not integer (int) then the quantizer can not be reversible.
   *
   * <p>After initializing member attributes, getAnSubbandTree is called for all components setting the 'stepWMSE' for all subbands in the current tile.</p>
   *
  ...
  /**
   * Returns true if the quantized data is reversible, for the specified tile-component. For the quantized data to be reversible it is necessary and sufficient
   * that the quantization is reversible.
   *
   ...
   *
   * <p>The data returned by this method is always a copy of the data. Therfore it can be modified "in place" without any problems after being returned. The
  ...
  /**
   * Converts the floating point value to its exponent-mantissa representation. The mantissa occupies the 11 least significant bits (bits 10-0), and the
   * exponent the previous 5 bits (bits 15-11).
   *
  ...
  /**
   * Converts the exponent-mantissa representation to its floating-point value. The mantissa occupies the 11 least significant bits (bits 10-0), and the
   * exponent the previous 5 bits (bits 15-11).
   *
