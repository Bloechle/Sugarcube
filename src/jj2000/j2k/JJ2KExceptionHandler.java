...
/**
 * This class handles exceptions. It should be used in places where it is not known how to handle the exception, and the exception can not be thrown higher in
 * the stack.
 *
 * <P>Different options can be registered for each Thread and ThreadGroup. <i>This feature is not implemented yet</i>
 *
 */
public class JJ2KExceptionHandler
{
  /**
   * Handles the exception. If no special action is registered for the current thread, then the Exception's stack trace and a descriptive message are printed to
   * standard error and the current thread is stopped.
   *
