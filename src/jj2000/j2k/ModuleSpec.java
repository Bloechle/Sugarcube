...
/**
 * This generic class is used to handle values to be used by a module for each tile and component. It uses attribute to determine which value to use. It should
 * be extended by each module needing this feature.
 *
 * This class might be used for values that are only tile specific or component specific but not both.
 *
 * <p>The attributes to use are defined by a hierarchy. The hierarchy is:
 *
 * <ul> <li> Tile and component specific attribute</li> <li> Tile specific default attribute</li> <li> Component main default attribute</li> <li> Main default
 * attribute</li> </ul></p>
 *
 */
public class ModuleSpec implements Cloneable
{
  ...
  /**
   * The specific value for each tile-component. Value of tile 16 component 3 is accessible through the hash value "t16c3". Null if no tile-component specific
   * value is defined
   */
   ...
   *
   * @param nT Number of tiles along horizontal and vertical axis after rotation. 
     *
   */
  ...
  /**
   * Constructs a 'ModuleSpec' object, initializing all the components and tiles to the 'SPEC_DEF' spec val type, for the specified number of components and
   * tiles.
   *
  ...
  /**
   * Gets value of specified tile-component without knowing if a specific tile-component value has been previously entered. It first check if a tile-component
   * specific value has been entered, then if a tile specific value exist, then if a component specific value exist. If not the default value is returned.
   *
   * @param t Tile index
   *
   * @param c Component index
  ...
  /**
   * This method is responsible of parsing tile indexes set and component indexes set for an option. Such an argument must follow the following policy:<br>
   *
   * <tt>t&lt;indexes set&gt;</tt> or <tt>c&lt;indexes set&gt;</tt> where tile or component indexes are separated by commas or a dashes.
   *
   * <p><u>Example:</u><br> <li> <tt>t0,3,4</tt> means tiles with indexes 0, 3 and 4.<br> <li> <tt>t2-4</tt> means tiles with indexes 2,3 and 4.<br>
   *
