 ...
 *
 * <p>The code-blocks are rectangular and their dimensions must be powers of 2. Each dimension cannot be smaller than 4 and larger than 256. The product of the
 * two dimensions (i.e. area of the code-block) cannot exceed 4096.</p>
 *
  ...
  implements StdEntropyCoderOptions
{
  ...
  /**
   * The options that are turned on, as flag bits. The options are 'OPT_TERM_PASS', 'OPT_RESET_MQ', 'OPT_VERT_STR_CAUSAL', 'OPT_BYPASS' and 'OPT_SEG_SYMBOLS' as
   * defined in the StdEntropyCoderOptions interface
   *
  ...
  /**
   * The state array for entropy coding. Each element of the state array stores the state of two coefficients. The lower 16 bits store the state of a
   * coefficient in row 'i' and column 'j', while the upper 16 bits store the state of a coefficient in row 'i+1' and column 'j'. The 'i' row is either the
   * first or the third row of a stripe. This packing of the states into 32 bit words allows a faster scan of all coefficients on each coding pass and
   * diminished the amount of data transferred. The size of the state array is increased by 1 on each side (top, bottom, left, right) to handle boundary
   * conditions without any special logic.
   *
   ...
   *
   * <P>The upper 16 bits the state is stored as in the lower 16 bits, but with the bits shifted up by 16.
   *
   * <P>The lower 16 bits are referred to as "row 1" ("R1") while the upper 16 bits are referred to as "row 2" ("R2").
     *
   */
   ...
   *
   * @param opt The options to use for this encoder. It is a mix of the 'OPT_TERM_PASS', 'OPT_RESET_MQ', 'OPT_VERT_STR_CAUSAL', 'OPT_BYPASS' and
   * 'OPT_SEG_SYMBOLS' option flags.
   *
   * @param doer If true error detection will be performed, if any error detection features have been enabled.
   *
   * @param verber This flag indicates if the entropy decoder should be verbose about bit stream errors that are detected and concealed.
   *
   * @param mQuit the maximum number of bit planes to decode according to the m quit condition
     *
   */
...
//      System.runFinalizersOnExit(true);
   ...
   * same code-block will return the exact same data values.
   *
   ...
   * after being returned. The 'offset' of the returned data is 0, and the 'scanw' is the same as the code-block width. See the 'DataBlk' class.
   *
  ...
  /**
   * Performs the significance propagation pass on the specified data and bit-plane. It decodes all insignificant samples which have, at least, one of its
   ...
   *
   * <p>This method also checks for segmentation markers if those are present and returns true if an error is detected, or false otherwise. If an error is
   * detected it means that the bit stream contains some erroneous bit that have led to the decoding of incorrect data. This data affects the whole last decoded
   * bit-plane (i.e. 'bp'). If 'true' is returned the 'conceal' method should be called and no more passes should be decoded for this code-block's bit
   * stream.</p>
   *
   ...
   *
   * @param isterm If this pass has been terminated. If the pass has been terminated it can be used to check error resilience.
   *
   ...
   * immediate eight neighbors already significant, using the ZC and SC primitives as needed. It toggles the "visited" state bit to 1 for all those samples.
   *
   * <p>This method bypasses the arithmetic coder and reads "raw" symbols from the bit stream.</p>
   *
   * <p>This method also checks for segmentation markers if those are present and returns true if an error is detected, or false otherwise. If an error is
   * detected it measn that the bit stream contains some erroneous bit that have led to the decoding of incorrect data. This data affects the whole last decoded
   * bit-plane (i.e. 'bp'). If 'true' is returned the 'conceal' method should be called and no more passes should be decoded for this code-block's bit
   * stream.</p>
   *
   * @param cblk The code-block data to decode
  ...
  /**
   * Performs the magnitude refinement pass on the specified data and bit-plane. It decodes the samples which are significant and which do not have the
   * "visited" state bit turned on, using the MR primitive. The "visited" state bit is not mofified for any samples.
   *
   * <P>This method also checks for segmentation markers if those are present and returns true if an error is detected, or false otherwise. If an error is
   * detected it means that the bit stream contains some erroneous bit that have led to the decoding of incorrect data. This data affects the whole last decoded
   * bit-plane (i.e. 'bp'). If 'true' is returned the 'conceal' method should be called and no more passes should be decoded for this code-block's bit stream.
   *
   * @param cblk The code-block data to decode
   *
   ...
   * "visited" state bit turned on, using the MR primitive. The "visited" state bit is not mofified for any samples.
   *
   * <P>This method bypasses the arithmetic coder and reads "raw" symbols from the bit stream.
   *
   * <P>This method also checks for segmentation markers if those are present and returns true if an error is detected, or false otherwise. If an error is
   * detected it measn that the bit stream contains some erroneous bit that have led to the decoding of incorrect data. This data affects the whole last decoded
   * bit-plane (i.e. 'bp'). If 'true' is returned the 'conceal' method should be called and no more passes should be decoded for this code-block's bit stream.
  ...
  /**
   * Performs the cleanup pass on the specified data and bit-plane. It decodes all insignificant samples which have its "visited" state bit off, using the ZC,
   * SC, and RLC primitives. It toggles the "visited" state bit to 0 (off) for all samples in the code-block.
   *
   * <P>This method also checks for segmentation markers if those are present and returns true if an error is detected, or false otherwise. If an error is
   * detected it measn that the bit stream contains some erroneous bit that have led to the decoding of incorrect data. This data affects the whole last decoded
   * bit-plane (i.e. 'bp'). If 'true' is returned the 'conceal' method should be called and no more passes should be decoded for this code-block's bit stream.
      ...
      for (; sk < stopsk; sk++, sj++)
      {
        ...
        top_half:
        {
            ...
            && sheight == STRIPE_HEIGHT)
          {
            ...
            // and a sugarcube.app.sample became significant
            // Use sign coding
            if ((rlclen & 0x01) == 0)
            {
  ...
  /**
   * Conceals decoding errors detected in the last bit-plane. The concealement resets the state of the decoded data to what it was before the decoding of
   * bit-plane 'bp' started. No more data should be decoded after this method is called for this code-block's data to which it is applied.
   *
  ...
  private void conceal(DataBlk cblk, int bp)
  {
    ...
    // Visit each sugarcube.app.sample, apply the reset mask to it and add an
