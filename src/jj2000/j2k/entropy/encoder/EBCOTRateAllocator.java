...
/**
 * This implements the EBCOT post compression rate allocation algorithm. This algorithm finds the most suitable truncation points for the set of code-blocks,
 * for each layer target bitrate. It works by first collecting the rate distortion info from all code-blocks, in all tiles and all components, and then running
 * the rate-allocation on the whole image at once, for each layer.
 *
...
public class EBCOTRateAllocator extends PostCompRateAllocator
{
   ...
   *
   * <ul> <li>1st index: tile index</li> <li>2nd index: component index</li> <li>3rd index: resolution level index</li> <li>4th index: subband index</li>
   * <li>5th index: code-block index</li> </ul>
     *
   */
  private CBlkRateDistStats cblks[][][][][];
  /**
   * 6D Array containing the indices of the truncation points. It actually contains the index of the element in CBlkRateDistStats.truncIdxs that gives the real
   * truncation point index.
   *
   * <ul> <li>1st index: tile index</li> <li>2nd index: layer index</li> <li>3rd index: component index</li> <li>4th index: resolution level index</li> <li>5th
   * index: subband index</li> <li>6th index: code-block index</li> </ul>
     *
   */
   ...
   *
   * <ul> <li>1st dim: tile index.</li> <li>2nd dim: component index.</li> <li>3nd dim: resolution level index.</li> </ul>
     *
   */
  ...
  /**
   * The R-D summary information collected from the coding of all code-blocks. For each entry it contains the accumulated length of all truncation points that
   * have a slope not less than '2*(k-RD_SUMMARY_OFF)', where 'k' is the entry index.
   *
   * <p>Therefore, the length at entry 'k' is the total number of bytes of code-block data that would be obtained if the truncation slope was chosen as
   * '2*(k-RD_SUMMARY_OFF)', without counting the overhead associated with the packet heads.</p>
   *
   * <p>This summary is used to estimate the relation of the R-D slope to coded length, and to obtain absolute minimums on the slope given a length. </p>
   */
...
//      System.runFinalizersOnExit(true);
  ...
  /**
   * Initializes the layers array. This must be called after the main header has been entirely written or simulated, so as to take its overhead into account.
   * This method will get all the code-blocks and then initialize the target bitrates for each layer, according to the specifications.
     *
   */
  public void initialize() throws IOException
  {
    ...
    while (newbytes < minlsz)
    {
      ...
      {
        if (newbytes <= 0)
          throw new IllegalArgumentException("Overall target bitrate too "
    ...
    if (DO_TIMING)
      initTime += System.currentTimeMillis() - stime;
  }

  /**
   * This method gets all the coded code-blocks from the EBCOT entropy coder for every component and every tile. Each coded code-block is stored in a 5D array
   * according to the component, the resolution level, the tile, the subband it belongs and its position in the subband.
   *
   * <P> For each code-block, the valid slopes are computed and converted into the mantissa-exponent representation.
     *
   */
  ...
  /**
   * This method builds all the bit stream layers and then writes them to the output bit stream. Firstly it builds all the layers by computing the threshold
   * according to the layer target bit-rate, and then it writes the layer bit streams according to the progressive type.
     *
   */
  private void buildAndWriteLayers() throws IOException
  {
    ...
    if (DO_TIMING)
      buildTime += System.currentTimeMillis() - stime;
    ...
    if (DO_TIMING)
      writeTime += System.currentTimeMillis() - stime;
  }
    ...
    int lys[][], int lye) throws IOException
  {
    ...
    { //loop on resolution levels
      ...
      for (int l = minlys; l < lye; l++) //loop on layers
        ...
        } // End loop on components // End loop on layers
    } // End loop on resolution levels
  }
    ...
    int[][] lys, int lye) throws IOException
  {
    ...
    for (int l = minlys; l < lye; l++) // loop on layers
      for (int r = rs; r < re; r++) // loop on resolution level
        ...
        } // end loop on components // end loop on resolution levels // end loop on layers
  }
  ...
  /**
   * This function implements the rate-distortion optimization algorithm. It saves the state of any previously generated bit-stream layers and then simulate the
   * formation of a new layer in the bit stream as often as necessary to find the smallest rate-distortion threshold such that the total number of bytes
   * required to represent the layer does not exceed `maxBytes' minus `prevBytes'. It then restores the state of any previously generated bit-stream layers and
   * returns the threshold.
   *
   * @param layerIdx The index of the current layer
   *
   * @param fmaxt The maximum admissible slope value. Normally the threshold slope of the previous layer.
   *
   * @param maxBytes The maximum number of bytes that can be written. It includes the length of the current layer bistream length and all the previous layers
   * bit streams.
   *
  ...
  /**
   * This function finds the new truncation points indices for a packet. It does so by including the data from the code-blocks in the component, resolution
   * level and tile, associated with a R-D slope which is larger than or equal to 'fthresh'.
   *
   * @param layerIdx The index of the current layer
   *
   ...
   *
   * @param subb The LL subband in the resolution level lvlIdx, which is parent of all the subbands in the packet. Except for resolution level 0 this subband is
   * always a node.
   *
  ...
  /**
   * Returns the index of a slope for the summary table, limiting to the admissible values. The index is calculated as RD_SUMMARY_OFF plus the maximum exponent,
   * base 2, that yields a value not larger than the slope itself.
   *
   * <p>If the value to return is lower than 0, 0 is returned. If it is larger than the maximum table index, then the maximum is returned.</p>
   *
