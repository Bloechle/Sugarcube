...
/**
 * This class implements the JPEG 2000 entropy coder, which codes stripes in code-blocks. This entropy coding engine can function in a single-threaded mode
 * where one code-block is encoded at a time, or in a multi-threaded mode where multiple code-blocks are entropy coded in parallel. The interface presented by
 * this class is the same in both modes.
 *
 * <p>The number of threads used by this entropy coder is specified by the "jj2000.j2k.entropy.encoder.StdEntropyCoder.nthreads" Java system property. If set to
 * "0" the single threaded implementation is used. If set to 'n' ('n' larger than 0) then 'n' extra threads are started by this class which are used to encode
 * the code-blocks in parallel (i.e. ideally 'n' code-blocks will be encoded in parallel at a time). On multiprocessor machines under a "native threads" Java
 * Virtual Machine implementation each one of these threads can run on a separate processor speeding up the encoding time. By default the single-threaded
 * implementation is used. The multi-threaded implementation currently assumes that the vast majority of consecutive calls to 'getNextCodeBlock()' will be done
 * on the same component. If this is not the case, the speed-up that can be expected on multiprocessor machines might be significantly decreased.</p>
 *
 * <p>The code-blocks are rectangular, with dimensions which must be powers of 2. Each dimension has to be no smaller than 4 and no larger than 256. The product
 * of the two dimensions (i.e. area of the code-block) may not exceed 4096.</p>
 *
 * <p>Context 0 of the MQ-coder is used as the uniform one (uniform, non-adaptive probability distribution). Context 1 is used for RLC coding. Contexts 2-10 are
 * used for zero-coding (ZC), contexts 11-15 are used for sign-coding (SC) and contexts 16-18 are used for magnitude-refinement (MR).</p>
 *
 * <p>This implementation buffers the symbols and calls the MQ coder only once per stripe and per coding pass, to reduce the method call overhead.</p>
 *
 * <p>This implementation also provides some timing features. They can be enabled by setting the 'DO_TIMING' constant of this class to true and recompiling. The
 * timing uses the 'System.currentTimeMillis()' Java API call, which returns wall clock time, not the actual CPU time used. The timing results will be printed
 * on the message output. Since the times reported are wall clock times and not CPU usage times they can not be added to find the total used time (i.e. some
 * time might be counted in several places). When timing is disabled ('DO_TIMING' is false) there is no penalty if the compiler performs some basic
 * optimizations. Even if not the penalty should be negligeable.</p>
 *
 * <p>The source module must implement the CBlkQuantDataSrcEnc interface and code-block's data is received in a CBlkWTData instance. This modules sends
  ...
  implements StdEntropyCoderOptions
{
  ...
  /**
   * The cumulative wall time for the entropy coding engine, for each component. In the single-threaded implementation it is the total time, in the
   * multi-threaded implementation it is the time spent managing the compressor threads only.
   */
  private long time[];
  /**
  ...
  /**
   * The increase in priority for the compressor threads, currently 3. The compressor threads will have a priority of THREADS_PRIORITY_INC more than the
   * priority of the thread calling this class constructor. Used only in the multi-threaded implementation.
   */
  ...
  /**
   * The options that are turned on, as flag bits. One element for each tile-component. The options are 'OPT_TERM_PASS', 'OPT_RESET_MQ', 'OPT_VERT_STR_CAUSAL',
   * 'OPT_BYPASS' and 'OPT_SEG_SYMBOLS' as defined in the StdEntropyCoderOptions interface
   *
  ...
  /**
   * Sign Coding context lookup table. The index into the table is a 9 bit index, which correspond the the value in the 'state' array shifted by 'SC_SHIFT'.
   * Bits 8-5 are the signs of the horizontal-left, horizontal-right, vertical-up and vertical-down neighbors, respectively. Bit 4 is not used (0 or 1 makes no
   * difference). Bits 3-0 are the significance of the horizontal-left, horizontal-right, vertical-up and vertical-down neighbors, respectively. The least 4
   * bits of the value in the lookup table define the context number and the sign bit defines the "sign predictor".
   */
  ...
  /**
   * The state array for each thread. Each element of the state array stores the state of two coefficients. The lower 16 bits store the state of a coefficient
   * in row 'i' and column 'j', while the upper 16 bits store the state of a coefficient in row 'i+1' and column 'j'. The 'i' row is either the first or the
   * third row of a stripe. This packing of the states into 32 bit words allows a faster scan of all coefficients on each coding pass and diminished the amount
   * of data transferred. The size of the state array is increased by 1 on each side (top, bottom, left, right) to handle boundary conditions without any
   * special logic.
   *
   * <p>The state of a coefficient is stored in the following way in the lower 16 bits, where bit 0 is the least significant bit. Bit 15 is the significance of
   * a coefficient (0 if non-significant, 1 otherwise). Bit 14 is the visited state (i.e. if a coefficient has been coded in the significance propagation pass
   * of the current bit-plane). Bit 13 is the "non zero-context" state (i.e. if one of the eight immediate neighbors is significant it is 1, otherwise is 0).
   * Bits 12 to 9 store the sign of the already significant left, right, up and down neighbors (1 for negative, 0 for positive or not yet significant). Bit 8
   * indicates if the magnitude refinement has already been applied to the coefficient. Bits 7 to 4 store the significance of the left, right, up and down
   * neighbors (1 for significant, 0 for non significant). Bits 3 to 0 store the significance of the diagonal coefficients (up-left, up-right, down-left and
   * down-right; 1 for significant, 0 for non significant).</p>
   *
   * <p>The upper 16 bits the state is stored as in the lower 16 bits, but with the bits shifted up by 16.</p>
   *
   * <p>The lower 16 bits are referred to as "row 1" ("R1") while the upper 16 bits are referred to as "row 2" ("R2").</p>
     *
   */
  private int stateT[][];
  ...
  /**
   * The flag bit for the "not zero context" bit in the state array, for row 1. This bit is always the OR of bits STATE_H_L_R1, STATE_H_R_R1, STATE_V_U_R1,
   * STATE_V_D_R1, STATE_D_UL_R1, STATE_D_UR_R1, STATE_D_DL_R1 and STATE_D_DR_R1.
   */
  ...
  /**
   * The flag bit for the "not zero context" bit in the state array, for row 2. This bit is always the OR of bits STATE_H_L_R2, STATE_H_R_R2, STATE_V_U_R2,
   * STATE_V_D_R2, STATE_D_UL_R2, STATE_D_UR_R2, STATE_D_DL_R2 and STATE_D_DR_R2.
   */
  ...
  /**
   * Distortion estimation lookup table for bits coded using the sign-code (SC) primative, for lossless coding and last bit-plane. This table is different from
   * 'fs_lossy' since when doing lossless coding the residual distortion after the last bit-plane is coded is strictly 0.
   */
  private static final int FS_LOSSLESS[] = new int[1 << (MSE_LKP_BITS - 1)];
  /**
   * Distortion estimation lookup table for bits coded using the magnitude-refinement (MR) primative, for lossless coding and last bit-plane. This table is
   * different from 'fs_lossless' since when doing lossless coding the residual distortion after the last bit-plane is coded is strictly 0.
   */
  ...
  private class Compressor implements Runnable
  {
    ...
    /**
     * Calls the 'compressCodeBlock()' method with thread local arguments. Once completed it adds itself to the 'completedComps[c]' stack, where 'c' is the
     * component for which this compressor is running. This last step occurs even if exceptions are thrown by the 'compressCodeBlock()' method.
         *
     */
    public void run()
    {
    ...
    /**
     * Returns the wall time spent by this compressor for component 'c' since the last call to this method (or the creation of this compressor if not yet
     * called). If DO_TIMING is false 0 is returned.
     *
     * @return The wall time in milliseconds spent by this compressor since the last call to this method.
         *
     */
   ...
   *
   * <p>If the 'OPT_PRED_TERM' option is given then the MQ termination must be 'TERM_PRED_ER' or an exception is thrown.</p>
   *
...
//      System.runFinalizersOnExit(true);
  ...
  public void finalize() throws Throwable
  {
    if (DO_TIMING)
    {
      ...
      { // Multithreaded implementation
        ...
        Enumeration enume = idleComps.elements();
        sb.append("\nStdEntropyCoder compressor threads wall clock "
          + "time:");
        while (enume.hasMoreElements())
        {
          compr = (Compressor) (enume.nextElement());
          for (c = 0; c < time.length; c++)
          {
            sb.append("\n  compressor ");
   ...
   *
   * @return The next coded code-block in the current tile for component 'n', or null if all code-blocks for the current tile have been returned.
   *
   * @see CBlkRateDistStats
     *
   */
   ...
   *
   * <p>This default implementation just advances to the next tile in the source.</p>
     *
   */
   ...
   * @param c The component for which to return the next code-block.
   *
   * @param ccb The object where the compressed data will be stored. If the 'data' array of 'cbb' is not null it may be reused to return the compressed data.
   *
   * @param srcblk The code-block data to code
   *
   * @param mq The MQ-coder to use
   *
   * @param bout The bit level output to use. Used only if 'OPT_BYPASS' is turned on in the 'options' argument.
   *
   * @param out The byte buffer trough which the compressed data is stored.
   *
   * @param state The state information for the code-block
   *
   * @param distbuf The buffer where to store the distortion at the end of each coding pass.
   *
   * @param ratebuf The buffer where to store the rate (i.e. coded lenth) at the end of each coding pass.
   *
   * @param istermbuf The buffer where to store the terminated flag for each coding pass.
   *
   ...
   *
   * @param rev The reversible flag. Should be true if the source of this code-block's data is reversible.
   *
    ...
    int lcType, int tType)
  {
    ...
    if ((options & OPT_PRED_TERM) != 0 && tType != MQCoder.TERM_PRED_ER)
      throw new IllegalArgumentException("Embedded error-resilient info "
  ...
  /**
   * Calculates the number of magnitude bit-planes that are to be skipped, because they are non-significant. The algorithm looks for the largest magnitude and
   * calculates the most significant bit-plane of it.
   *
   * @param cblk The code-block of data to scan
   *
   * @param lmb The least significant magnitude bit in the data
   *
   * @return The number of magnitude bit-planes to skip (i.e. all zero most significant bit-planes).
     *
   */
  ...
  /**
   * Performs the significance propagation pass on the specified data and bit-plane. It codes all insignificant samples which have, at least, one of its
   * immediate eight neighbors already significant, using the ZC and SC primitives as needed. It toggles the "visited" state bit to 1 for all those samples.
   *
   * @param srcblk The code-block data to code
   *
   * @param mq The MQ-coder to use
   *
   * @param doterm If true it performs an MQ-coder termination after the end of the pass
   *
   ...
   * @param ctxtbuf A buffer to hold the contexts to use in sending the buffered symbols to the MQ coder.
   *
   * @param ratebuf The buffer where to store the rate (i.e. coded lenth) at the end of this coding pass.
   *
   * @param pidx The coding pass index. Is the index in the 'ratebuf' array where to store the coded length after this coding pass.
   *
   * @param ltpidx The index of the last pass that was terminated, or negative if none.
   *
   * @param options The bitmask of entropy coding options to apply to the code-block
   *
   * @return The decrease in distortion for this pass, in the fixed-point normalized representation of the 'FS_LOSSY' and 'FS_LOSSLESS' tables.
     *
   */
    ...
    int options)
  {
    ...
    int normval;     // The normalized sugarcube.app.sample magnitude value
  ...
  /**
   * Performs the significance propagation pass on the specified data and bit-plane, without using the arithmetic coder. It codes all insignificant samples
   * which have, at least, one of its immediate eight neighbors already significant, using the ZC and SC primitives as needed. It toggles the "visited" state
   * bit to 1 for all those samples.
   *
   * <p>In this method, the arithmetic coder is bypassed, and raw bits are directly written in the bit stream (useful when distribution are close to uniform,
   * for intance, at high bit-rates and at lossless compression).</p>
   *
   * @param srcblk The code-block data to code
   *
   * @param bout The bit based output
   *
   * @param doterm If true the bit based output is byte aligned after the end of the pass.
   *
   * @param bp The bit-plane to code
   *
   * @param state The state information for the code-block
  ...
  /**
   * Performs the magnitude refinement pass on the specified data and bit-plane. It codes the samples which are significant and which do not have the "visited"
   * state bit turned on, using the MR primitive. The "visited" state bit is not mofified for any samples.
   *
   * @param srcblk The code-block data to code
   *
   * @param mq The MQ-coder to use
  ...
  /**
   * Performs the magnitude refinement pass on the specified data and bit-plane, without using the arithmetic coder. It codes the samples which are significant
   * and which do not have the "visited" state bit turned on, using the MR primitive. The "visited" state bit is not mofified for any samples.
   *
   * <p>In this method, the arithmetic coder is bypassed, and raw bits are directly written in the bit stream (useful when distribution are close to uniform,
   * for intance, at high bit-rates and at lossless compression). The 'STATE_PREV_MR_R1' and 'STATE_PREV_MR_R2' bits are not set because they are used only when
   * the arithmetic coder is not bypassed.</p>
   *
   * @param srcblk The code-block data to code
   *
   * @param bout The bit based output
  ...
  /**
   * Performs the cleanup pass on the specified data and bit-plane. It codes all insignificant samples which have its "visited" state bit off, using the ZC, SC,
   * and RLC primitives. It toggles the "visited" state bit to 0 (off) for all samples in the code-block.
   *
   * @param srcblk The code-block data to code
   *
   * @param mq The MQ-coder to use
      ...
      for (nsym = 0; sk < stopsk; sk++, sj++)
      {
        ...
        top_half:
        {
            ...
            && sheight == STRIPE_HEIGHT)
          {
            ...
            ctxtbuf[nsym++] = UNIF_CTXT;
            // Code sign of sugarcube.app.sample that became significant
            // Update distortion
            normval = (data[k] >> downshift) << upshift;
            dist += fs[normval & ((1 << (MSE_LKP_BITS - 1)) - 1)];
            // Apply sign coding
            ...
            if ((rlclen & 0x01) == 0)
            {
              ...
              if ((rlclen >> 1) != 0)
                ...
                break top_half;
              // Otherwise sugarcube.app.sample that became significant is in
   ...
   *
   * <p>Due to error resiliance reasons, a coding pass should never have its last byte be a 0xFF, since that can lead to the emulation of a resync marker. This
   * method checks if that is the case, and reduces the rate for a given pass if necessary. The ommitted 0xFF will be synthetized by the decoder if necessary,
   * as required by JPEG 2000. This method should only be called once that the entire code-block is coded.</p>
   *
   * <p>Passes that are terminated are not checked for the 0xFF byte, since it is assumed that the termination procedure does not output any trailing 0xFF.
   * Checking the terminated segments would involve much more than just modifying the stored rates.</p>
   *
   * <p>NOTE: It is assumed by this method that the coded data does not contain consecutive 0xFF bytes, as is the case with the MQ and 'arithemetic coding
   * bypass' bit stuffing policy. However, the termination policies used should also respect this requirement.</p>
   *
   * @param data The coded data for the code-block
   *
   * @param rates The rate (i.e. accumulated number of bytes) for each coding pass
   *
   * @param isterm An array of flags indicating, for each pass, if it is terminated or not. If null it is assumed that no pass is terminated, except the last
   * one.
   *
  ...
  public void initTileComp(int nt, int nc)
  {
    ...
    for (int t = 0; t < nt; t++)
      ...
      } // End loop on components // End loop on tiles
  }
   ...
   *
   * @return The precinct partition width for the specified component, tile and resolution level
     *
   */
   ...
   * @param rl the resolution level
   *
   * @return The precinct partition height for the specified component, tile and resolution level
     *
   */
   ...
   * @param t The tile
   *
   * @return True if precinct partition is used for the specified component and tile, returns false otherwise.
     *
   */
