...
/**
 * This is the abstract class from which post-compression rate allocators which generate layers should inherit. The source of data is a 'CodedCBlkDataSrcEnc'
 * which delivers entropy coded blocks with rate-distortion statistics.
 *
 * <p>The post compression rate allocator implementation should create the layers, according to a rate allocation policy, and send the packets to a
 * CodestreamWriter. Since the rate allocator sends the packets to the bit stream then it should output the packets to the bit stream in the order imposed by
 * the bit stream profiles.</p>
 *
...
public abstract class PostCompRateAllocator extends ImgDataAdapter
{
  ...
  /**
   * Initializes the rate allocation points, taking into account header overhead and such. This method must be called after the header has been simulated but
   * before calling the runAndWrite() one. The header must be rewritten after a call to this method since the number of layers may change.
   *
  ...
  /**
   * Creates a PostCompRateAllocator object for the appropriate rate allocation parameters in the parameter list 'pl', having 'src' as the source of entropy
   * coded data, 'rate' as the target bitrate and 'bw' as the bit stream writer object.
   *
   * @param src The source of entropy coded data.
   *
   ...
   *
   * @param bw The bit stream writer object, where the bit stream data will be written.
     *
   */
    ...
    EncoderSpecs encSpec)
  {
    ...
    LayersInfo lyrs = parseAlayers(pl.value("Alayers"), rate);
