...
/**
 * This class provides an adapter to perform bit based output on byte based output objects that inherit from a 'ByteOutputBuffer' class. This class implements
 * the bit stuffing policy needed for the 'selective arithmetic coding bypass' mode of the entropy coder. This class also delays the output of a trailing 0xFF,
 * since they are synthetized be the decoder.
 *
 */
class BitToByteOutput
{
  ...
  int bbuf;
  /**
   * The position of the next bit to put in the bit buffer. When it is 7 the bit buffer 'bbuf' is empty. The value should always be between 7 and 0 (i.e. if it
   * gets to -1, the bit buffer should be immediately written to the byte output).
   */
  int bpos = 7;
  /**
  ...
  /**
   * Returns the length, in bytes, of the output bit stream as written by this object. If the output bit stream does not have an integer number of bytes in
   * length then it is rounded to the next integer.
   *
