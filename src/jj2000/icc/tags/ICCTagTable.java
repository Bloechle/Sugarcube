 ...
 *
 * On disk the tag table exists as a byte array conventionally aggragted into a structured sequence of types (bytes, shorts, ints, and floats. The first four
 * bytes are the integer count of tags in the table. This is followed by an array of triplets, one for each tag. The triplets each contain three integers, which
 * are the tag signature, the offset of the tag in the byte array and the length of the tag in bytes. The tag data follows. Each tag consists of an integer (4
 * bytes) tag type, a reserved integer and the tag data, which varies depending on the tag.
 *
...
public class ICCTagTable extends Hashtable
{
  ...
  protected ICCTagTable(byte[] data)
  {
    ...
    Enumeration enume = trios.elements();
    while (enume.hasMoreElements())
    {
      Triplet trio = (Triplet) enume.nextElement();
