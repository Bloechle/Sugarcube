...
import jj2000.colorspace.boxes.PaletteBox;
...
/**
 * This class provides decoding of images with palettized colorspaces. Here each sugarcube.app.sample in the input is treated as an index into a color palette of triplet sRGB
 * output values.
 *
...
public class PalettizedColorSpaceMapper extends ColorSpaceMapper
{
  ...
  private void initialize() throws ColorSpaceException
  {
    if (ncomps != 1 && ncomps != 3)
      throw new ColorSpaceException("PalettizedColorSpaceMapper.initialize - wrong number of components (" + ncomps + ") for palettized image");
   ...
   *
   * <P>The returned data has its 'progressive' attribute set to that of the input data.
   *
   ...
   *
   * @param c The index of the component from which to get the data. Only 0 and 3 are valid.
   *
  ...
  /**
   * Return a suitable String representation of the class instance, e.g. <p> [PalettizedColorSpaceMapper ncomps= 3, scomp= 1, nentries= 1024 column=0, 7 bit
   * signed entry column=1, 7 bit unsigned entry column=2, 7 bit signed entry] <p>
   *
   */
