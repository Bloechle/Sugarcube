...
import jj2000.colorspace.boxes.*;
...
/**
 * This class analyzes the image to provide colorspace information for the decoding chain. It does this by examining the box structure of the JP2 image. It also
 * provides access to the parameter list information, which is stored as a public final field.
 *
...
public class ColorSpace
{
  ...
  final protected void getBoxes() throws ColorSpaceException, IOException
  {
    ...
    for (boxStart += 8; boxStart < headerBoxEnd; boxStart += len)
    {
      ...
      len = (long) ICCProfile.getInt(boxHeader, 0);
      if (len == 1)
        throw new ColorSpaceException("Extended length boxes "
          + "not supported");
  ...
  /**
   * Return a palettized sugarcube.app.sample
   *
   * @param channel requested
   * @param index of entry
   * @return palettized sugarcube.app.sample
   */
   ...
   */
  public final static CSEnum CMYK = new CSEnum("CMYK");
  /**
   * colorspace enumeration
   */
